...
| --- | --- | --- |
| Source Fast Gate | Cheap deterministic checks required on each source PR and repeated on the synthetic combined head. | `Path Changes` (both), `ci-fast` (both), `Secret Scan (gitleaks + trufflehog)` (both), `Golden Path Lock` (both), `Visual Snapshot Compare` (both), `Migration Guard` (both), `Unit Tests` (merge-group) |
...
| `Visual Snapshot Compare` | both | fast-gate | `node scripts/visual-snapshot-compare.mjs compare` |

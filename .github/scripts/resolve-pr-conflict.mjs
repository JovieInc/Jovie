...
function main() {
  ...
  const prJson = gh(['pr', 'view', String(PR_NUMBER), '--repo', REPO,
  ...
  console.log(`   Base: ${pr.baseRefName} | Branch: ${pr.headRefName} | Mergeable: ${pr.mergeable}`);
  ...
  if (pr.mergeable !== 'CONFLICTING') {
    ...
    const hasLabel = pr.labels.some(l => l.name === 'needs-conflict-resolution');
    if (hasLabel && !DRY_RUN) {
      gh(['pr', 'edit', String(PR_NUMBER), '--repo', REPO, '--remove-label', 'needs-conflict-resolution']);
  ...
  try {
    ...
    execSync(`git fetch origin ${pr.baseRefName}`, { cwd: tmpDir, encoding: 'utf-8', timeout: 60_000 });

    // Set identity
    execSync('git config user.email "gem@jovie.app"', { cwd: tmpDir, timeout: 10_000 });
    execSync('git config user.name "Gem (CI Bot)"', { cwd: tmpDir, timeout: 10_000 });

    // Try the merge
    try {
      execSync('git merge --no-edit FETCH_HEAD', { cwd: tmpDir, encoding: 'utf-8', timeout: 60_000 });
      console.log('   ✅ Merge clean!');

      if (!DRY_RUN) {
        execSync(`git push origin ${pr.headRefName}`, { cwd: tmpDir, encoding: 'utf-8', timeout: 60_000 });
        gh(['pr', 'edit', String(PR_NUMBER), '--repo', REPO, '--remove-label', 'needs-conflict-resolution']);
      ...
      // Real merge conflict
      const files = execSync('git diff --name-only --diff-filter=U', { cwd: tmpDir, encoding: 'utf-8' })
        .trim().split('\n').filter(Boolean);
...
These conflicts need manual resolution.`;
        ...
        gh(['pr', 'comment', String(PR_NUMBER), '--repo', REPO, '--body', comment]);

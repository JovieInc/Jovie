...
- Pre-land CHANGELOG prohibition (JOV-5378): implementation PRs never edit
  `CHANGELOG.md`. Source CI rejects the diff, and native admission independently
  skips any legacy candidate that still touches the file. User-visible changes
  earn exactly one What's New bullet only after land/runtime proof through the
  release/UI path. This is a classified skip, not an `enroll` product failure.

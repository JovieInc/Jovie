...
`ownerless-recovery-sweep.yml` is a hosted fallback independent of Gem, Symphony, and fleet health. Every 15 minutes it may promote a PR unassigned for at least one hour only when the exact head is
same-repo, current with `main`, unstacked, conflict-free, focused to CI, DevEx, delivery control,
or waitlist-canary files, and has passing focused checks. Incomplete patches and credential,
privacy, destructive, production-promotion, or check-bypass changes fail closed. It records an
attempt receipt before mutation; success requires the same head to be merged or to have a positive authoritative native queue position.

...
function Image({
  ...
  // Resolve public-path srcs (e.g. "/brand-logos/x.png") to inlined data URIs so
  // they render in the preview sandbox (map injected by prebuild.mjs banner).
  const assetMap =
    (globalThis as { __DS_PUBLIC_ASSETS?: Record<string, string> })
      .__DS_PUBLIC_ASSETS ?? {};
  const resolvedSrc =
    typeof rawSrc === 'string' && assetMap[rawSrc] ? assetMap[rawSrc] : rawSrc;

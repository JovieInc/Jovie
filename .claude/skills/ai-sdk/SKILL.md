...
Jovie's repository policy takes precedence over the upstream defaults below. Inspect the versions already declared in `apps/web/package.json`; the workspace install may live under `apps/web/node_modules` or be resolved by pnpm rather than at root `node_modules/ai`. Do not install or upgrade `ai`, provider packages, or model IDs unless the task explicitly includes dependency work. Route application calls through `apps/web/lib/ai/sdk.ts`, preserving Jovie's gateway, model-selection, leak-guard, cost, and eval contracts.
...
1. Confirm `ai` is declared in the relevant workspace package and resolve its installed location through the project's package manager. A missing root `node_modules/ai/` is not evidence that a monorepo dependency is absent. Outside Jovie, if the dependency is truly undeclared, install **only** the `ai` package using the project's package manager. Install provider packages and framework packages later, when the task requires them.

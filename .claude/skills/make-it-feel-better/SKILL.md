...
8. **Cinematic only for reveals:** drawers, audio bar, modals use `duration-cinematic ease-cinematic`.
...
- `apps/web/eslint-rules/no-raw-motion-values.js` for `transition-all`, raw ms, ease-in, scale(0), decorative hover motion, `will-change: all`
- `apps/web/tests/unit/design-system/*-ratchet.test.ts` for shrink-only counters

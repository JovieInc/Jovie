# Skill resolver (unique leaf names)

First rung of the gbrain scaling-skills pattern: one catalog line per unique name.
Nested `.bak` / `.cursor` / `.factory` / `src` / `test` / `bin` copies are not catalog-visible.
`.claude/skills/<name>` and `.agents/skills/gstack/<name>` that share a name are one leaf.

- **ai-sdk**: .claude/skills/ai-sdk/SKILL.md | .agents/skills/ai-sdk/SKILL.md
- **auth-console-sync**: .claude/skills/auth-console-sync/SKILL.md
- **autoplan**: .claude/skills/autoplan/SKILL.md | .agents/skills/gstack/autoplan/SKILL.md
- **benchmark**: .agents/skills/gstack/benchmark/SKILL.md
- **brand-deals**: .claude/skills/brand-deals/SKILL.md | .agents/skills/brand-deals/SKILL.md
- **browse**: .agents/skills/gstack/browse/SKILL.md
- **canary**: .agents/skills/gstack/canary/SKILL.md
- **careful**: .agents/skills/gstack/careful/SKILL.md
- **clerk-cli**: .claude/skills/clerk-cli/SKILL.md
- **codex**: .agents/skills/gstack/codex/SKILL.md
- **connect-chrome**: .agents/skills/gstack/connect-chrome/SKILL.md
- **cso**: .agents/skills/gstack/cso/SKILL.md
- **design-canonical**: .claude/skills/design-canonical/SKILL.md | .agents/skills/gstack/design-canonical/SKILL.md
- **design-consultation**: .agents/skills/gstack/design-consultation/SKILL.md
- **design-html**: .agents/skills/gstack/design-html/SKILL.md
- **design-review**: .agents/skills/gstack/design-review/SKILL.md
- **design-shotgun**: .agents/skills/gstack/design-shotgun/SKILL.md
- **document-release**: .agents/skills/gstack/document-release/SKILL.md
- **eve**: .claude/skills/eve/SKILL.md
- **find-skills**: .claude/skills/find-skills/SKILL.md | .agents/skills/find-skills/SKILL.md
- **freeze**: .agents/skills/gstack/freeze/SKILL.md
- **gstack**: .agents/skills/gstack/SKILL.md
- **gstack-upgrade**: .agents/skills/gstack/gstack-upgrade/SKILL.md
- **guard**: .agents/skills/gstack/guard/SKILL.md
- **idea-radar**: .claude/skills/idea-radar/SKILL.md
- **investigate**: .agents/skills/gstack/investigate/SKILL.md
- **jovie-performance-hardening**: .claude/skills/jovie-performance-hardening/SKILL.md
- **land-and-deploy**: .agents/skills/gstack/land-and-deploy/SKILL.md
- **last30days**: .claude/skills/last30days/SKILL.md
- **lavish**: .claude/skills/lavish/SKILL.md
- **learn**: .agents/skills/gstack/learn/SKILL.md
- **load-investors**: .claude/skills/load-investors/SKILL.md
- **nightly-test-agent**: .agents/skills/nightly-test-agent/SKILL.md
- **office-hours**: .agents/skills/gstack/office-hours/SKILL.md
- **perf-loop**: .agents/skills/gstack/perf-loop/SKILL.md
- **plan-ceo-review**: .agents/skills/gstack/plan-ceo-review/SKILL.md
- **plan-design-review**: .agents/skills/gstack/plan-design-review/SKILL.md
- **plan-eng-review**: .agents/skills/gstack/plan-eng-review/SKILL.md
- **qa**: .agents/skills/gstack/qa/SKILL.md
- **qa-only**: .agents/skills/gstack/qa-only/SKILL.md
- **qa-swarm**: .claude/skills/qa-swarm/SKILL.md
- **retro**: .agents/skills/gstack/retro/SKILL.md
- **review**: .agents/skills/gstack/review/SKILL.md
- **setup-browser-cookies**: .agents/skills/gstack/setup-browser-cookies/SKILL.md
- **setup-deploy**: .agents/skills/gstack/setup-deploy/SKILL.md
- **setup-gbrain**: .claude/skills/setup-gbrain/SKILL.md
- **ship**: .agents/skills/gstack/ship/SKILL.md
- **sync-gbrain**: .claude/skills/sync-gbrain/SKILL.md
- **tool-discovery**: .claude/skills/tool-discovery/SKILL.md
- **unfreeze**: .agents/skills/gstack/unfreeze/SKILL.md
- **vercel-composition-patterns**: .claude/skills/vercel-composition-patterns/SKILL.md | .agents/skills/vercel-composition-patterns/SKILL.md
- **vercel-react-best-practices**: .claude/skills/vercel-react-best-practices/SKILL.md | .agents/skills/vercel-react-best-practices/SKILL.md
- **youtube-thumbnail-growth**: .agents/skills/youtube-thumbnail-growth/SKILL.md

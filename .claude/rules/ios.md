...
- Keep device identifiers straight. The observed gstack device daemon requires the CoreDevice identifier from `xcrun devicectl list devices`; `xcodebuild -destination` and libimobiledevice use the hardware UDID, while `devicectl` accepts several selectors including either identifier. Do not assume every tool accepts the same value.
- When project-level defaults disable signing, a local physical Debug build must explicitly use `CODE_SIGNING_ALLOWED=YES CODE_SIGN_STYLE=Automatic CODE_SIGN_IDENTITY='Apple Development' DEVELOPMENT_TEAM=<team-id>`. Read the team ID from the installed development profile or signing identity; do not guess it from a certificate label. Add `-allowProvisioningUpdates` only when provisioning is missing and Apple Developer account mutation is explicitly authorized; never add `-allowProvisioningDeviceRegistration` implicitly.
- Separate evidence tiers in every report: a preserved real session on the physical device, a physical-device fixture launch, simulator automation, and source/unit tests prove different things. Fixture routes must never be reported as authenticated or server-backed success.
- A post-login `.needsOnboarding` route must offer core profile completion natively. A generic "complete/continue on web" detour or fallback fails device acceptance unless the action is explicitly labeled and proven to be a genuinely external or web-only workflow.
- Treat `Timed out while enabling automation mode` as an inconclusive XCTest harness/bootstrap failure, not an app pass or app failure. Preserve the `.xcresult` and device logs; classify it as beta-OS/tooling infrastructure only after signing, app/runner launch, and logs rule out an app or runner fault. Use a read-only device capture path such as QuickTime when possible, and leave interaction-dependent criteria unproven until a deterministic input channel is available.
- Any in-app QA bridge must be `#if DEBUG`/test-only, bind only to loopback or the USB tunnel, expose no credentials or session data, be absent from Release artifacts, and be removed after the pass. Smoke-test it with one read and one reversible navigation on an inert fixture before a long pass. If either request times out or blocks the main actor, stop after the first ambiguous mutation, relaunch, and fall back; never retry a possibly dispatched tap or production-backed submission blindly.

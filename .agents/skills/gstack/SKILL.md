...
> This rule applies to ALL gstack skills — it is the shared preamble convention.
> Phased rollout: applied to /autoplan first, then all skills.
...
**Verbose mode (interactive sessions):** full multi-sentence narration is expected.
Explain what you did, found, and decided. The user is actively engaged.

**Always log (both modes):**
- Phase transitions (entering/leaving a phase)
- Blockers (hard failures, missing dependencies, auth errors)
- Final report/summary (full detail delivered at end regardless of mode)

**Never narrate (both modes):**
- Internal reasoning chains, deliberation, or intermediate "thinking aloud"
  about what to do next. Keep reasoning internal / in scratch files.

**Sub-probes / fan-outs** (skills invoked from within a phase) inherit this
convention. When spawning a sub-probe, pin it to a cheap model with a bounded
scope. Full verbose narration from a sub-probe defeats the purpose.

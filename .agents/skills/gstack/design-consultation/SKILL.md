...
For writing or voice work, load `~/.claude/skills/gstack/references/VOICE.md` on demand. Host instructions, user preferences and repo canon govern; this reference creates no new approval gate.
...
Removed. Use in-repo Playwright (`pnpm exec playwright`), not the gstack browse daemon.
...
- Display/Hero: obey the loaded project's `DESIGN.md`; for Jovie use Satoshi only
- Body: obey the loaded project's `DESIGN.md`; for Jovie use Inter only
...
**Overused fonts** (never recommend as primary unless the loaded design authority
already selected one): Roboto, Arial, Helvetica, Open Sans, Lato, Montserrat,
Poppins

For Jovie, serif product typography is banned. Never recommend a serif as a
creative risk after loading Jovie's `DESIGN.md`.

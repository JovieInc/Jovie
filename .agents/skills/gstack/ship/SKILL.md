...
If `TEL_PROMPTED` is `no`, do **not** ask telemetry questions or write prompt-state markers from this skill. `/ship` stays non-interactive. Defer telemetry onboarding to an interactive skill and continue immediately.
...
- Create one required Linear issue using `~/.claude/skills/gstack/review/LINEAR-followups.md`, include the
...
> CI (`scripts/version-fanout-guard.mjs`) will fail the PR if you do. Do not edit
> `CHANGELOG.md`; the post-land release/UI path owns the single user-visible
> What's New bullet when runtime proof says one is warranted.

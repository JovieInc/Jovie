    ...
    <div className='space-y-0.5' aria-busy='true'>
      {skeletonKeys.map(skeletonKey => (
        <div
          ...
          aria-hidden='true'
        >
          <div className='absolute left-3 top-0 bottom-0 w-px bg-(--color-border-subtle)' />
...
function ActivityEventRow({ event }: { readonly event: ActivityEvent }) {
  ...
  return (
    <div className='group relative flex items-start gap-3 rounded-md px-1.5 py-1 transition-[background-color] duration-subtle hover:bg-surface-1 focus-within:bg-surface-1'>
      <div
        aria-hidden='true'
        className='absolute left-3 top-0 bottom-0 w-px bg-(--color-border-subtle) group-last:hidden'
      />

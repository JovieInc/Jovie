...
interface EditLinkModalProps {
  ...
  onSave: (id: string, updates: { title: string; url: string; platform: Platform }) => Promise<void>;
}
...
export function EditLinkModal({ isOpen, onClose, link, onSave }: EditLinkModalProps) {
  ...
  const handlePlatformChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Auto-update title based on platform
    if (title === '' || platformOptions.some(p => p.label === title)) {
      const platformLabel = platformOptions.find(p => p.value === newPlatform)?.label || 'Custom Link';

...
import { AudienceMobileCard } from '@/components/dashboard/organisms/AudienceMobileCard';
    ...
    >
      <div className='shrink-0 border-b border-subtle bg-surface-1/75 backdrop-blur-md'>
        <div className='flex flex-wrap items-start justify-between gap-4 px-4 py-4 sm:px-6'>
          <div>
            ...
            <p className='mt-1 text-sm leading-6 text-secondary-token hidden sm:block'>
              {mode === 'members'
        ...
        <div
          className={cn(
            'lg:hidden border-t border-subtle px-4 py-2 flex items-center justify-between gap-2 transition-all duration-200',
            selectedCount > 0
      ...
      <div className='flex-1 min-h-0 overflow-hidden'>
        <div className='flex h-full min-h-0 flex-col bg-surface-1'>
          <div className='flex-1 min-h-0 overflow-auto' ref={tableContainerRef}>
            {rows.length === 0 ? (
              ...
              <>
                ...
                <div className='lg:hidden px-4 py-4 space-y-3'>
                  {rows.map(row => (
                    <AudienceMobileCard
                      ...
                      onCardClick={() => setSelectedMember(row)}
                      mode={mode}
                    />
                  ))}
                </div>
                ...
                <table
                  className='hidden lg:table w-full min-w-[960px] border-separate border-spacing-0 text-[13px]'
                  aria-label={
                    mode === 'members'
                ...
                >
                  ...
                  >
                    {rowVirtualizer.getVirtualItems().map(virtualRow => {
                      ...
                      const deviceIndicator = getDeviceIndicator(
                        row.deviceType
                      );
                              ...
                              <td className='px-4 py-3 align-middle text-sm text-primary-token sm:px-6'>
                                <div className='flex items-center gap-2'>
                                  {row.latestActions
                                    ...
                                    .map((action, idx) => {
                                      const iconName =
                                        resolveAudienceActionIcon(action.label);
                                      return (
                                        <span
          ...
          <div className='sticky bottom-0 z-20 flex flex-wrap items-center justify-between gap-2 border-t border-subtle bg-surface-1/75 px-4 py-2 text-xs text-secondary-token backdrop-blur-md sm:gap-3 sm:px-6'>
            <span className='tracking-wide'>
              <span className='hidden sm:inline'>{paginationLabel()}</span>
            ...
            <div className='flex items-center gap-2 sm:gap-3'>
              ...
              <div className='flex items-center gap-1 sm:gap-2'>
                <Button
                  ...
                  className='h-9 w-9 p-0 rounded-md border border-subtle bg-transparent text-secondary-token hover:bg-surface-2 hover:text-primary-token sm:h-auto sm:w-auto sm:px-3 sm:py-1.5'
                ...
                <span className='text-xs text-tertiary-token sm:hidden'>
                  {page}/{totalPages}
                </span>
                <Button
                  variant='ghost'

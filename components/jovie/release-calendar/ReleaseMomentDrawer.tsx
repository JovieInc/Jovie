        ...
        >
          <SheetHeader>
            <div className='flex items-center gap-2'>
              <span className='rounded-full border border-(--color-border-subtle) bg-(--linear-bg-surface-1) px-2 py-0.5 text-3xs font-semibold uppercase tracking-wide text-(--color-text-secondary-token)'>
                {MOMENT_LABEL[moment.momentType]}
              </span>
              <span className='text-xs text-(--color-text-tertiary-token)'>
          ...
          <section className='flex flex-col gap-2'>
            <h3 className='text-xs font-semibold uppercase tracking-wide text-(--color-text-secondary-token)'>
              Workflow
            </h3>
              ...
              {workflowTaskSlugsForMoment(moment.momentType).map(slug => {
                ...
                return (
                  <li
                    ...
                    className='flex items-start justify-between gap-2 rounded-md border border-(--color-border-subtle) bg-(--linear-bg-surface-1) px-3 py-2'
                  >
                    <div className='flex flex-col gap-0.5'>
                      <span className='text-sm text-(--color-text-primary-token)'>
                        <span className='mr-2 font-mono text-xs text-(--color-text-tertiary-token)'>
                          {formatRelativeDays(task.relativeDays)}
                        </span>
                        · <span className='font-medium'>{task.title}</span>
                      </span>
                      <span className='text-2xs text-(--color-text-tertiary-token)'>
          ...
          <SheetClose
            ...
            className='self-end rounded-md border border-(--color-border-subtle) bg-(--linear-bg-surface-1) px-3 py-1.5 text-sm text-(--color-text-primary-token) hover:bg-(--linear-bg-surface-2)'
          >
            Close
          </SheetClose>
        </SheetContent>
...
function FanNotificationPreview({ moment }: { readonly moment: DemoMoment }) {
  ...
  return (
    <section
      ...
      className='flex flex-col gap-1.5 rounded-md border border-(--color-border-subtle) bg-(--linear-bg-surface-1) px-3 py-3'
    >
      ...
      <p className='text-sm font-medium text-(--color-text-primary-token)'>
        {notif.headline}
      </p>
      <p className='text-xs text-(--color-text-secondary-token)'>{notif.body}</p>
      <p className='text-2xs text-(--color-text-tertiary-token)'>

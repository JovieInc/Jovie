...
const STATUS_DOT: Record<string, string> = {
  ...
  missing: 'var(--color-text-tertiary-token)',
      ...
      <div className='flex-1 overflow-y-auto p-4'>
        ...
        <div className='mb-8 space-y-1'>
          <DemoPropertyRow label='Status'>
            <div className='-ml-2 flex items-center gap-1.5 rounded px-2 py-1'>
        ...
        <div className='mt-4'>
          ...
          <DrawerSurfaceCard className='space-y-0.5 border-0 shadow-none bg-surface-0 p-2'>
            {DEMO_ACTIVITY_TEMPLATE.map((activity, index) => (
              <div
                key={activity.id}
                className='relative flex items-start gap-2.5 rounded px-1 py-1.5 pr-1'
              >
                {index < DEMO_ACTIVITY_TEMPLATE.length - 1 ? (
                  <div
                    aria-hidden='true'
                    className='absolute bottom-0 left-3 top-0 w-px bg-(--color-border-subtle)/65'
                  />
                ) : null}
                <div className='relative z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-surface-0'>
                  <div className='size-1 rounded-full bg-(--color-text-tertiary-token)/80' />
                </div>

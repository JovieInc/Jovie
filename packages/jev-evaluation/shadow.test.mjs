...
it('contains synchronous and rejected asynchronous model failures without certifying', async () => {
  for (const evaluate of [
    () => {
      throw new Error('synthetic provider error');
    },
    () => Promise.reject(new Error('synthetic async error')),
    () => ({
      then(_resolve, reject) {
        reject(new Error('synthetic thenable error'));
      },
    }),
  ]) {
    const shadow = classifyJevShadow({
    ...
    assert.equal(shadow.alignment, 'insufficient');
    assert.equal(shadow.certified, false);
    assert.ok(!JSON.stringify(shadow).includes('synthetic'));

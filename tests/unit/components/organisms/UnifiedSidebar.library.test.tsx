...
describe('UnifiedSidebar library route', () => {
  ...
  it('uses existing subtle border token without growing --linear-* namespace', () => {
    ...
    // --linear-border-subtle retired to canonical --color-border-subtle (JOV-5458);
    // assert the canonical token is defined in both light and dark blocks.
    expect(designSystem).toMatch(/--color-border-subtle:\s*oklch\(0% 0 0 \/ 5%\);/);
    expect(linearTokens).toMatch(
      /--linear-app-frame-seam:\s*rgba\(0, 0, 0, 0\.045\);/
    );
    expect(designSystem).toMatch(
      /:root\.dark[\s\S]*--color-border-subtle:\s*var\(--noir-ion-border-subtle\);/
    );
    expect(linearTokens).toMatch(

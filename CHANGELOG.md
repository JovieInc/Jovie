...
- **[auth] Better Auth OTP error handling + account settings foundation (PR #13900)**: Email OTP send/verify surfaces Better Auth `{ data, error }` failures instead of silently advancing; account settings shows a read-only Better Auth identity summary with mobile-safe wrapping; desktop native auth smoke and packaged Info.plist validation harden the Electron handoff path.

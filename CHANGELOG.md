...
- [internal] **Per-workflow model A/B bake-off + cost-aware auto-promotion (GH-11462)**: New `model_experiments` / `model_usage_events` / `model_promotions` tables, deterministic weighted traffic splits for the chat workflow, 👍/👎 quality attribution per model, per-request token/cost logging, daily auto-promotion under the equal-or-cheaper cost rule (cost regressions escalate as `needs_decision`), and an admin API (`/api/admin/model-experiments`) for create/pause/rollback + per-arm stats.

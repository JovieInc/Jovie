...
- [internal] **Founder design corrections now have an append-only evidence ledger:** typed lifecycle, provenance, conflict checks, promotion thresholds, model-review receipts, negative examples, supersession, and rollback keep one-off feedback from silently rewriting canon.
...
- [internal] **Symphony landing throughput is measured from exact lineage:** complete GitHub merge evidence now attributes only canonical `symphony/JOV-N-fix` branches, keeps idle UTC hours in the distribution, reports hourly and inter-landing percentiles, and fails the 5-PRs/hour reliability gate instead of hiding downtime behind active-hour averages.
...
- **Sidebar sections group through spacing instead of divider lines (JOV-4652):** Chats and the identity footer keep their existing rhythm, scroll behavior, and anchoring without visible horizontal separator rules.
- **Profile and DSP detail rails share one entity header:** artist identity, platform context, missing-image treatment, and the close affordance now use the canonical rail header instead of separate compact layouts.
- [internal] **Shared marketing primitives retire residual System A styling (JOV-4165):** the voice demo and FAQ now use canonical System B surfaces, typography, focus, motion, and visibility contracts with reduced-motion and source guards.
...
- **[internal] Beat grids preserve both analyzer output and artist intent (JOV-4385):** variable-tempo beats, downbeats, bar segments, revisions, and adoption decisions are typed and mutation-tested, and analyzer reruns never silently overwrite a user-edited grid.
...
- [internal] **gstack pinned per run + out-of-band nightly upgrade (JOV-4184)**: preflight no longer invokes `gstack-update-check` in-run (reads the cached state file instead; policy defaults to `pinned`). Upgrades move to the nightly Hermes cron `scripts/hermes/jobs/gstack-nightly-upgrade.ts` with backup/restore and ops alerts on failure.
...
- **[internal] Stripe webhook processing leases (JOV-4191)**: duplicate deliveries now claim an atomic five-minute processing lease before invoking billing handlers; active duplicates are acknowledged without double-applying billing state, failures release the lease, and crashed workers recover after expiry.
...
- **[auth] Better Auth OTP error handling + account settings foundation (PR #13900)**: Email OTP send/verify surfaces Better Auth `{ data, error }` failures instead of silently advancing; account settings shows a read-only Better Auth identity summary with mobile-safe wrapping and a11y/overflow hardening on the OTP flow.

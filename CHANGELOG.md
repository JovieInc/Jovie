...
- [internal] **Temporary production auth-smoke advisory policy:** authenticated release smoke still runs and leaves the controller visibly red on failure, while exact deployment verification can proceed only when every other production gate passes. Authoritative controller history graduates the smoke back to blocking after 10 consecutive exact-main passes.

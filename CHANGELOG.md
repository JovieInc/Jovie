...
- **[internal] Background jobs fail unhealthy (JOV-4192)**: ingestion and workflow cron endpoints now return non-2xx responses when jobs fail or workflow tables are unavailable, so monitoring and retry infrastructure cannot mistake a partial or unconfigured tick for success.
- **[internal] Trial access uses canonical entitlement semantics (JOV-4193)**: active trials now receive the same Pro-level access as paid users in verification, SMS access, and promo-download flows, while expired trials remain denied.
- **[internal] Memory graph RLS isolation (JOV-3061)**: all memory graph tables now enforce tenant ownership at the database layer, including join tables, with a compatibility resolver for Better Auth UUIDs and legacy Clerk IDs during migration.

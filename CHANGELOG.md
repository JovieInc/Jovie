...
- **Audio playback now reports real transition and latency states (JOV-3689, JOV-4356):** the singleton player distinguishes loading, buffering, seeking, stalls, interruptions, completion, and errors; keeps Media Session position current; records play-to-audible, buffer-recovery, and seek-settlement measures; and removes a timing-dependent first-progress edge.
- **Audio uploads now keep their format intact across Library, Chat, and promo downloads (JOV-4362):** browsers that omit or alias MIME types are normalized before upload, while unsupported and oversized promo files are rejected again on the server with a clear rule.

...
- **Desktop signed-out launch (GH-12822)**: Packaged Mac apps now recover to the in-window `/desktop-auth` sign-in surface when an expired session redirects `/app/chat` to `/signin`, instead of stranding users on a blank/black window. Staging and Local builds register `jovie-staging://` and `jovie-local://` deep-link schemes so browser auth returns to the correct installed shell.

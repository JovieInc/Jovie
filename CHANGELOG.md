...
- [internal] **Codex issue shipper checkout guard (#12841)**: fail-closed `stale_checkout_abort` when the primary ~/Jovie checkout is not on `origin/main`; embedded gate in `shipper-gated-entrypoint.py`, TS guard for direct runs, optional detritus-only auto-recover, Telegram/Slack alerts, and a bash hook blocking `git checkout` in the primary repo.

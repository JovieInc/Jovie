...
- **[internal] Audio analyzers now use a canonical provider registry (JOV-4385):** OpenKeyScan and Essentia capabilities, adapters, licensing gates, and adoption status are typed; provenance rejects unknown engines and production selection remains fail-closed until a provider passes the real-media benchmark and approval contract.
...
- **[internal] Real-media audio compatibility rail (JOV-4387):** a hash-pinned, synthetic fixture corpus now covers every canonical format and truncated-container failure in real Chromium on each combined queue head, while production-player tests preserve the active track and playhead across shell consumer transitions. Upload acceptance, MIME capability, direct decode support, and app-shell ownership remain separate proof layers.
...
- **Timed lyrics now activate from canonical LRC timestamps (JOV-4388):** the full lyrics surface parses line and enhanced word timing with typed provenance, metadata, offsets, repeated cues, and millisecond timestamps on the server. Mixed or malformed timing falls back to readable lyrics, inactive tracks cannot seek unrelated audio, active cues stay visible, and the timeline has keyboard slider semantics without changing layout geometry.
- **Playback now communicates real transition states across the app (JOV-3689):** loading waits for audible playback, buffering and stalled media get distinct status messages, cue jumps publish seeking state immediately, interruptions remain nested-safe, and finished tracks resolve through the canonical typed playback contract.

...
- [internal] **gstack pinned per run + out-of-band nightly upgrade (JOV-4184)**: preflight no longer invokes `gstack-update-check` in-run (reads the cached state file instead; policy defaults to `pinned`). Upgrades move to the nightly Hermes cron `scripts/symphony/jobs/gstack-nightly-upgrade.ts` with backup/restore and ops alerts on failure.
...
- [internal] Schedule the codex kanban ship loop via Houston launchd (`co.jovie.hermes.cron-codex-kanban-ship`, every 15m) with PAUSE-respecting `scripts/symphony/ship-loop.sh`.

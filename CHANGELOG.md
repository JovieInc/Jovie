...
- [internal] **Rolling CI Dispatch binds the exact head into its planner payload (JOV-5377):** authenticated `workflow_run` completions now pass a 40-character `headSha` into the Node planner instead of null jq shorthand, so exact-head classification and green recovery can run.

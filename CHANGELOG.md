...
- [internal] **The packaged Mac completes Ovie turns through current Summer (JOV-5212):** the founder-authenticated desktop session claims durable Eve-bound turns, invokes the fixed Summer Hermes profile with founder text on stdin only, and posts fenced completion or failure. Idle polling continues on the next heartbeat, with no alternate speaker fallback.

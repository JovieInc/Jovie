...
## [Unreleased]
- [internal] **Pen work now fails closed on a pinned canonical file lock (JOV-4958):** a versioned workspace profile prevents side files from self-declaring canonical; saved-state receipts bind pre/post editor evidence, an explicit acknowledged save, clean window state, and root readback while reporting crash durability as unproven; lifecycle loss, path drift, and ambiguous save prompts stop without asking the founder to guess whether unknown work belongs.

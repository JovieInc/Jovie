...
- [internal] **Better Auth identity resolution + rate-limit TTL self-heal (#13958)**: `userIdentityFilter` matches clerk_id / better_auth_user_id / app UUID in shared user queries (fixes the /app ↔ /signin streamed-redirect loop and a uuid-cast throw in `getUserByIdentity`); secondary-storage `increment` applies `EXPIRE NX` so TTL-less rate-limit counters self-heal; op timeout env-tunable via `AUTH_SECONDARY_STORAGE_TIMEOUT_MS`. Desktop QA gates: Better Auth native smoke + packaged Info.plist validation matrix (no desktop runtime changes; no DMG release needed).

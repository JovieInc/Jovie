...
## [Unreleased]
- [internal] **Production waitlist monitoring is durable and non-destructive:** one exact retained Better Auth identity now traverses the real homepage-to-waitlist path, emits separately scoped runtime, database, analytics, and communications receipts, suppresses customer-facing waitlist email and Slack delivery, and never receives production database credentials or runs cleanup SQL.

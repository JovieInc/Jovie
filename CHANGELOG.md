...
- [internal] **Icon-only controls now have one canonical source owner (JOV-5307):** the cross-surface registry binds web `IconButton` to the native theme adapter, compatibility APIs must import and render that owner, and overflow/rail controls inherit its focus, hit-target, size, and interaction-state contract instead of owning parallel button chrome.

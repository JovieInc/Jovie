...
- [internal] **Redis quota warnings no longer auto-file as `Error: {"error":{"name":"UpstashError"}}` (JOV-5219):** `captureWarning(msg, { error })` unwraps the context bag so Sentry gets the real exception, and Upstash quota warnings stay breadcrumbs instead of Linear issues.

...
export async function invokeSummerRuntime(input: {
  ...
  const spawnProcess =
    input.spawnProcess ?? (spawn as unknown as SpawnSummer);
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    // Drain stderr so the child cannot block. It is never returned as founder
    // conversation text and may contain runtime diagnostics.
    ...
    try {
      ...
      try {
        ...
        receipt = {
          ...
          state: 'runtime-error',
          turnId: claimed.id,
          errorCode: error instanceof Error ? error.message : 'runtime-error',
        };
        return receipt;
      }

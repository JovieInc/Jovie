...
const PERF_ISSUES = [
  ...
  {
    id: 'P17',
    title: 'Former Releases workspace FCP was too slow',
    kind: 'timing-budget',
    surface: 'creator-app',
    evidence: [
      'Historical direct load for the former `/app/releases` workspace observed about 3032ms FCP.',
    ],
  ...
  {
    id: 'P18',
    title: 'Former Releases workspace request count was too high',
    kind: 'network-churn',
    surface: 'creator-app',
    evidence: [
      'Historical direct load for the former `/app/releases` workspace observed 94 requests.',
    ],
    batchId: 'B3-releases-workspace',
  ...
  {
    id: 'P19',
    title: 'Former Releases workspace JS and CSS payloads were too large',
    kind: 'resource-budget',
    surface: 'creator-app',
    evidence: [
      'Historical direct load for the former `/app/releases` workspace observed about 822KB JS and 159KB CSS.',
    ],
    batchId: 'B3-releases-workspace',
    checks: ['creator-releases-direct-metrics'],
  },
...
const PERF_BATCHES = [
  ...
  },
  {
    ...
    localChecks: [
      {
        ...
        expectation: 'pass',
        authPersona: 'creator-ready',
        routeIds: ['creator-library', 'creator-releases'],
      },
      {
        id: 'creator-releases-direct-metrics',
        label: 'Library releases-filter metrics hit the request and JS targets',
        kind: 'direct-metrics',
        expectation: 'pass',
        authPersona: 'creator-ready',
        path: `${APP_ROUTES.LIBRARY}?view=releases`,
        directMetrics: {
          finalPath: `${APP_ROUTES.LIBRARY}?view=releases`,
          requests: 69,
          scriptKB: 700,
        },
      },
    ...
    qaScope: [
      'Re-run screenshot parity for the Library releases-filter view and sidebar flows.',
      'QA warm navigation into Library and the legacy `/app/releases` redirect.',
    ],
    shipLabels: ['testing'],
    doneRule:

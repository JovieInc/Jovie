...
import {
  ...
  NIGHTLY_AGENT_REDIS_TTL_SECONDS,
...
function buildCommittedReportMarkdown(
  ...
  const header = [
    ...
    '',
    workflowRunUrl
      ? `> Latest workflow run: [GitHub Actions](${workflowRunUrl})`
      : '> Latest workflow run: see `.github/workflows/nightly-testing-agent.yml`',
    '',
  ];
  return [
    ...header,
...
async function commandPublishStatus(
  repoRoot: string,
  args: Record<string, string>
): Promise<void> {
  const dir = outputDir(repoRoot, args);
  const inputDir = args['input-dir']
    ? resolveFromRoot(repoRoot, args['input-dir'])
    : dir;
  const skillDeltaPath = path.join(dir, 'skill-delta.json');
  const reportPath = path.join(dir, 'nightly-report.md');
  if (!existsSync(skillDeltaPath)) {
    throw new Error(`Missing skill delta at ${skillDeltaPath}`);
  }
  if (!existsSync(reportPath)) {
    throw new Error(`Missing nightly report at ${reportPath}`);
  }

  const skillDelta = readJson<NightlyAgentSkillDelta>(skillDeltaPath);
  const reports = loadJsonReports(inputDir);
  const suites = reports.flatMap(report => report.suites);
  const workflowConclusion = args['workflow-conclusion'] as
    | NightlyAgentStatus['workflowConclusion']
    | undefined;
  const status = buildNightlyAgentStatusFromSkillDelta(skillDelta, {
    suites,
  ...
  const snapshotPath = resolveFromRoot(
    repoRoot,
  ...
  writeText(
    reportDocPath,
    buildCommittedReportMarkdown(reportMarkdown, args['workflow-run-url'])
  );
  writeJson(snapshotPath, status);

  const redisUrl = process.env.UPSTASH_REDIS_REST_URL;
  const redisToken = process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!redisUrl || !redisToken) {
    console.warn(
      'UPSTASH_REDIS_REST_URL/TOKEN unavailable — skipping Redis publish.'
    );
    console.log('Wrote nightly agent status files without Redis publish.');
    return;
  }

  const redis = new Redis({ url: redisUrl, token: redisToken });
  try {
    await redis.set(NIGHTLY_AGENT_REDIS_KEY, JSON.stringify(status), {
      ex: NIGHTLY_AGENT_REDIS_TTL_SECONDS,
    });
    console.log('Published nightly agent status to Redis.');
  } catch (error) {
    console.warn('Failed to publish nightly agent status to Redis:', error);
  }
}
...
async function main(): Promise<void> {
  ...
  if (command === 'context') commandContext(repoRoot, args);
    ...
    await commandPublishStatus(repoRoot, args);

...
function validateConfig(env: NodeJS.ProcessEnv): void {
...
async function readReceipt(env: NodeJS.ProcessEnv, url: URL) {
...
test.describe('Synthetic Monitoring - production waitlist', () => {
  ...
  test('retains identity and proves waitlist suppression', async ({ page }) => {
    ...
    const durableUrl = new URL(preflightUrl);
    durableUrl.search = '';
    ...
    expect(durable.emailSha256).toBe(emailSha256);
    expect(durable.entryId).toBe(intake.body.entryId);

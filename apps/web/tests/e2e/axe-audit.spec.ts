...
test.describe('Axe WCAG 2.1 Compliance', () => {
  ...
  // Authenticated routes (only run if SMOKE_ONLY is not set)
  ...
  for (const route of authenticatedRoutes) {
    test.skip(
      ...
      async ({ page }) => {
        // TODO: Add authentication setup when auth test utils are ready
        // For now, these tests are skipped in smoke mode
        await page.goto(route.path);
        await page.waitForLoadState('networkidle');
...
test.describe('Axe Best Practices', () => {
  test('Homepage should follow best practices', async ({ page }) => {
    ...
    // Best practices are less strict - just log violations, don't fail
    if (results.violations.length > 0) {
    ...
    // For now, we just want to be aware of best practice violations
    // They won't fail the test

...
test.describe('Link Creation & Display @smoke', () => {
  test(
    'public profile displays seeded social links @smoke',
    async ({ page }, testInfo) => {
      ...
      try {
        ...
        const spotifyLink = page
          ...
          .locator('a[href*="spotify"], a[href*="spotify.com"]')
          .first();
        ...
        // Verify the link is an absolute external URL
        expect(
          href,
          'Spotify link should be an absolute external URL',
        ).toMatch(/^https?:\/\//);
  ...
  test(
    'social links container exists even with no links @smoke',
    async ({ page }, testInfo) => {
      const { getContext, cleanup } = setupPageMonitoring(page);
      ...
      try {
        ...
        const socialLinksSection = page.locator(
  ...
  test(
    'link URLs are properly formatted and external @smoke',
    async ({ page }, testInfo) => {
      const { getContext, cleanup } = setupPageMonitoring(page);

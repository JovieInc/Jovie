...
- A deterministic AI skill prompt adapter, covering release-pitch prompt builders, retouch style prompt guardrails, and deliberate-red rule-case floors (unresolved curator checklist, invented listen URL/@handle/private email, ambiguous retouch identity) with synthetic Luna Waves fixtures and no model, DB, or network calls.
- A deterministic AI chat tool prompt adapter, covering album-art, Canvas, and bio prompt/context builders with synthetic Luna Waves fixtures and no model, DB, or network calls.
- A deterministic album-art image-provider source-contract adapter, covering AI Gateway auth gating, model selection, square three-image request shape, image byte handling, chat-tool preflight order, rate-limit order, and manifest prompt redaction with no model, DB, storage, or network calls.
...
- A deterministic route-contract adapter for `POST /api/mobile/v1/chat/turns`, covering unauthenticated, invalid JSON, invalid-body variants, and the authenticated handler boundary without starting Next or Clerk.

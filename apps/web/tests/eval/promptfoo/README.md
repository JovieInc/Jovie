...
- A deterministic album-art image-provider source-contract adapter, covering xAI key gating, model selection, square three-image request shape, image byte handling, chat-tool preflight order, rate-limit order, and manifest prompt redaction with no model, DB, storage, or network calls.

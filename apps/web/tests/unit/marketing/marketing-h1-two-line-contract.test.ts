...
describe('marketing H1 two-visual-line contract', () => {
  it('covers both public shells and every shared marketing hero owner', () => {
    ...
    const posterStories = readFileSync(
      path.join(
        webRoot,
        'components/marketing/MarketingPosterHero.stories.tsx'
      ),
      'utf8'
    );
    ...
    expect(posterHero).toContain("variant='unstyled'");
    ...
    expect(posterStories).toContain('OverlongHeadline');
    expect(posterStories).not.toContain('Own your story');
  });

  it('does not land superseded Quiet-hero homepage copy', () => {
    const homepageCopy = readFileSync(
      path.join(webRoot, 'data/homepageLaunchCopy.ts'),
      'utf8'
    );
    expect(homepageCopy).not.toContain(

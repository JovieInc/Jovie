...
describe('handleIngestionJobFailure', () => {
  ...
  it('falls back to DEFAULT_MAX_ATTEMPTS (3) when job.maxAttempts is null', async () => {
    const { tx } = createUpdateTxMock();
    vi.spyOn(Math, 'random').mockReturnValue(0);
    // attempts=2 with an effective max of 3 -> still below the limit -> retry
    const job = makeJob({
      attempts: 2,
      maxAttempts: null as unknown as number,
    });

    await handleIngestionJobFailure(tx, job, new Error('transient blip'));
  ...
  it('abandons at attempts=3 when job.maxAttempts is null — pins DEFAULT_MAX_ATTEMPTS to exactly 3', async () => {
    const { tx, set } = createUpdateTxMock();
    // attempts=3 with the fallback max of 3 -> at the limit -> abandon, not retry.
    // Together with the retry-side case above (attempts=2), this pins the
    // fallback constant from both sides: raising it to 4+ fails here,
    // lowering it to <=2 fails the sibling test.
    const job = makeJob({
      attempts: 3,
      maxAttempts: null as unknown as number,
    });

    await handleIngestionJobFailure(tx, job, new Error('third strike'));
...
describe('resetJobForRetry', () => {
  it('resets attempts to 0, clears error/nextRunAt, and re-queues runAt to now', async () => {
    const { tx, set, where, returning } = createReturningTxMock([
      { id: 'job-5' },
    ]);

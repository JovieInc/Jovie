...
const VIDEOS: readonly YouTubeRecentVideo[] = [
  {
    videoId: 'a1',
    title: 'One',
    thumbnailUrl: 'https://i/a1.jpg',
    publishedAt: null,
  },
  {
    videoId: 'b2',
    title: 'Two',
    thumbnailUrl: 'https://i/b2.jpg',
    publishedAt: null,
  },
  {
    videoId: 'c3',
    title: 'Three',
    thumbnailUrl: 'https://i/c3.jpg',
    publishedAt: null,
  },
];
...
function makeDeps(overrides: Partial<ThumbnailPreviewDeps> = {}) {
  ...
  const deps: ThumbnailPreviewDeps = {
    ...
    generateRedo: vi.fn(
      async (video: YouTubeRecentVideo) => `https://blob/${video.videoId}.jpg`
    ),
    cache: {
...
describe('buildThumbnailPreview', () => {
  ...
  it('returns preview_only with zero model calls and no counts when generation is off', async () => {
    ...
    expect(deps.guards.recordChannelForIp).toHaveBeenCalledWith(
      INPUT.ip,
      'UC1'
    );
  });
  ...
  it('rejects unknown channels and channels with no public videos', async () => {
    const unknown = makeDeps({
      resolveChannel: vi.fn().mockResolvedValue(null),
    });
    await expect(
      buildThumbnailPreview(INPUT, unknown.deps)
    ).rejects.toBeInstanceOf(InvalidChannelError);

    const empty = makeDeps({ listVideos: vi.fn().mockResolvedValue([]) });
    await expect(
      buildThumbnailPreview(INPUT, empty.deps)
    ).rejects.toMatchObject({ code: 'no_videos' });
  });
  ...
  it('blocks burst, channel spread, visitor cap and channel cap with the right codes', async () => {
    ...
    await expect(
      buildThumbnailPreview(INPUT, burst.deps)
    ...
    await expect(
      buildThumbnailPreview(INPUT, spread.deps)
    ).rejects.toMatchObject({ code: 'channel_spread' });
    ...
    await expect(
      buildThumbnailPreview(INPUT, channel.deps)
    ).rejects.toMatchObject({ code: 'channel_limit' });
  });
...
describe('buildRedoPrompt', () => {
  it('locks the identity guardrail into every prompt', () => {
    ...
    expect(prompt).toMatch(
      /do not generate, replace, retouch, restyle or alter any face or body/
    );

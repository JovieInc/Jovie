...
const transcriptEntrySchema = z.object({
  questionId: z.enum(INTAKE_QUESTION_IDS),
...
export async function POST(request: Request) {
  try {
    ...
    // access decision has already been made (and any auto-accept slot
    // consumed), so we MUST NOT fail the user-facing response just because
    // the metadata write failed — that would render a "retry" screen even
    // though their slot has been taken. Log + continue.
    try {

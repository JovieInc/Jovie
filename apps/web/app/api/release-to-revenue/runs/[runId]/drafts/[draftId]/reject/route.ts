...
export async function POST(_request: Request, { params }: RouteParams) {
  ...
  try {
    const result = await rejectDistributionDraft({ runId, draftId, userId });
    if (!result.ok) {
      const status =
    ...
    logger.error('[release-to-revenue/drafts/reject] failed', err);
    await captureError('release-to-revenue draft reject failed', err, {

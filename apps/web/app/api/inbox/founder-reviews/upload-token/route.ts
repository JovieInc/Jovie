...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!query.success) {
      throw new FounderReviewError('invalid-founder-review-upload-query', 400);
    }
    ...
    const response = await handleUpload({
      ...
      onBeforeGenerateToken: async pathname => {
        ...
        return {
          ...
          tokenPayload: JSON.stringify({ userId: appUserId, ...query.data }),
        };
      },
      onUploadCompleted: async () => {
        // The review POST verifies and binds media metadata to its receipt.
      },
    });

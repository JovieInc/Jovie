          ...
          {SAVED_VIEWS.map(v => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-2 h-7 px-2 rounded-md text-xs transition-colors duration-subtle ease-out',
                  active
        ...
        <div className='flex items-center justify-between px-2 pt-3 pb-1'>
          ...
          {hasFilters(filters) && (
            <button
              type='button'
              onClick={onClearAll}
              className='text-3xs uppercase tracking-[0.06em] text-tertiary-token hover:text-primary-token transition-colors duration-subtle ease-out'
            >
              Clear
            </button>
          )}
...
function SectionHead({ label, inline }: { label: string; inline?: boolean }) {
  return (
    <p
      className={cn(
        'text-3xs uppercase tracking-[0.08em] text-quaternary-token font-semibold',
  ...
  return (
    <div className='pt-2'>
      <button
        type='button'
        onClick={() => setOpen(v => !v)}
        className='w-full flex items-center gap-1 px-2 py-1 text-3xs uppercase tracking-[0.06em] text-tertiary-token hover:text-primary-token transition-colors duration-subtle ease-out'
      >
        <ChevronRight
          className={cn(
...
function FacetRow({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'flex items-center gap-2 h-6 pl-3 pr-2 rounded-md text-xs transition-colors duration-subtle ease-out',
        active
          ? 'bg-surface-1/80 text-primary-token'
          : 'text-secondary-token hover:bg-surface-1/50 hover:text-primary-token'
      )}
    >
      ...
      <span
        className={cn(
          ...
          mono && 'font-mono text-2xs tracking-wide'
        )}
      >
        {label}
      </span>
      <span className='text-3xs tabular-nums text-quaternary-token'>
        {count}
      </span>
      {active && (
        <span
...
function TopBar({
  ...
  return (
    <header className='shrink-0 h-12 px-4 flex items-center gap-3 border-b border-(--linear-app-shell-border) bg-(--linear-bg-page)/95 backdrop-blur-xl'>
      ...
      <span className='text-xs text-quaternary-token tabular-nums'>
...
function GenerateButton() {
  return (
    <button
      type='button'
      className='inline-flex items-center gap-1.5 h-7 px-3 rounded-md text-xs font-medium bg-cyan-300 text-black hover:bg-cyan-200 transition-colors duration-subtle ease-out'
    >
      ...
      >
        ...
        <div className='flex flex-col gap-1 px-2.5 pt-2 pb-2.5 min-w-0'>
          <div className='flex items-center gap-1.5 min-w-0'>
            ...
            />
            <p
              className='text-xs font-medium text-primary-token truncate'
              style={{ letterSpacing: '-0.005em' }}
            >
              {asset.title}
            </p>
          </div>
          <div className='flex items-center gap-1.5 text-3xs text-quaternary-token'>
        ...
        <span className='absolute top-2 left-2 inline-flex items-center h-5 px-1.5 rounded text-3xs font-caption uppercase tracking-[0.06em] text-primary-token bg-black/55 backdrop-blur opacity-0 group-hover:opacity-100 transition-opacity duration-subtle ease-out'>
          Inspect
        </span>
      </button>
      <button
...
function Poster({ asset }: { asset: Asset }) {
  ...
  return (
    <div className='relative w-full overflow-hidden bg-black'>
      ...
      />
      {asset.durationSec && (
        <span className='absolute bottom-1.5 right-1.5 inline-flex items-center h-4 px-1.5 rounded text-3xs font-caption text-primary-token bg-black/60 backdrop-blur'>
        ...
        {assets.map(a => {
          ...
          return (
            ...
            <div
              ...
              className={cn(
                'group relative grid items-center px-2 py-1 text-xs text-secondary-token text-left transition-colors duration-subtle ease-out cursor-pointer outline-none focus-visible:ring-1 focus-visible:ring-cyan-400/40',
                selected
            ...
            >
              ...
              <span className='font-mono text-2xs text-tertiary-token tracking-wide'>
                {a.aspect}
              </span>
      ...
      {asset && (
        <div className='h-full overflow-y-auto'>
            ...
            {editing ? (
              <div className='flex flex-col gap-2'>
                ...
                <div className='flex items-center gap-1.5 pt-1'>
                  <button
                    ...
                    className='h-7 px-3 rounded-md text-xs bg-cyan-300 text-black hover:bg-cyan-200 transition-colors duration-subtle ease-out'
                  >
                    Save
                  </button>
                  <button
              ...
              >
                <div className='flex-1 min-w-0'>
                  <h2
                    className='text-mid font-semibold text-primary-token truncate'
                    style={{ letterSpacing: '-0.01em' }}
                  >
                    {asset.title}
                  </h2>
                ...
                <span className='text-3xs uppercase tracking-[0.06em] text-quaternary-token opacity-0 group-hover/title:opacity-100 transition-opacity duration-subtle ease-out'>
                  Edit
                </span>
              </button>
            )}
          ...
          <DrawerSection label='Generated by'>
            ...
            {asset.promptSeed && (
              <p className='mt-2 text-2xs text-tertiary-token leading-relaxed'>
                {asset.promptSeed}
              </p>
            )}
          </DrawerSection>
  ...
  return (
    <div className='h-full grid place-items-center px-6'>
      <div className='max-w-md text-center'>
        ...
        <h2
          className='mt-4 text-mid font-semibold text-primary-token'
          style={{ letterSpacing: '-0.01em' }}
        >
          Nothing matches yet
        </h2>

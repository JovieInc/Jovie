...
describe('YoutubeThumbnailPasteForm', () => {
  ...
  it('explains preview-only mode when redos are not open yet', async () => {
    ...
    render(
      <YoutubeThumbnailPasteForm applyHref='/signup' fetchImpl={fetchImpl} />
    );
    await user.type(
      screen.getByTestId('youtube-thumbnails-channel-input'),
    ...
    expect(
      screen.getByText(
        'Redos open soon. These are the three we would start with.'
      )
    ).toBeVisible();
    expect(screen.queryByText(/free redos left/)).toBeNull();
  });

  const errorCases: ReadonlyArray<readonly [number, string, string]> = [
    [
      400,
      'invalid_channel',
    ...
    [
      403,
      'datacenter',
      'That network is blocked. Try again from a normal connection.',
    ],
    [
      503,
      'youtube_data_api_unavailable',
      'YouTube lookups are paused right now. Try again shortly.',
    ],
    [
      500,
      'internal_error',
  ...
  it.each(
    errorCases.map(([status, code, message]) => ({ status, code, message }))
  )('maps $status/$code to plain-language copy', async ({
    status,
    code,
    message,
  }) => {

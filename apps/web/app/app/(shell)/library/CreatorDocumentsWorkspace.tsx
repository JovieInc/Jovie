...
export function CreatorDocumentsWorkspace({
  ...
  >(undefined);
  const [pendingDiscardCapture, setPendingDiscardCapture] = useState(false);
  const hasIdeaDraft =
    ideaStatus === 'saving' ||
    title.trim().length > 0 ||
    body.trim().length > 0;
  ...
  const selectDocument = useCallback(
    (documentId: string | null) => {
      ...
      setPendingSelectId(documentId);
    },
    [discardPersistedDrafts, hasUnsavedDraft, selectedId]
  );
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        open={pendingSelectId !== undefined || pendingDiscardCapture}
        onOpenChange={open => {
          if (!open) {
            setPendingSelectId(undefined);
            setPendingDiscardCapture(false);
          }
        }}

...
export default async function AlertsPage({ params }: AlertsPageProps) {
  ...
  // Browser Meta pixel (fbq) — same retargeting audience as the public
  // profile (JOV-5078).
  const metaPixelIds = [
    result.creatorMetaPixelId,
    env.JOVIE_FACEBOOK_PIXEL_ID,
  ...
  return (
    <>
      {metaPixelIds.length > 0 ? <MetaPixel pixelIds={metaPixelIds} /> : null}

...
export class PaymentHandler extends BaseSubscriptionHandler {
  ...
  async handle(context: WebhookContext): Promise<HandlerResult> {
    ...
    switch (event.type) {
      case 'invoice.payment_succeeded':
        return this.handlePaymentSucceeded(invoice, stripeEventId, stripeEventTimestamp);

      case 'invoice.payment_failed':
        return this.handlePaymentFailed(invoice, stripeEventId, stripeEventTimestamp);
   ...
   * @private
   */
  private async extractUserId(subscription: Stripe.Subscription): Promise<string | null> {
    ...
    if (!userId && typeof subscription.customer === 'string') {
      await logFallback('No user ID in subscription metadata for payment failure', {
        event: 'invoice.payment_failed',
      });
   ...
   * @private
   */
  private isFailureStatus(status: Stripe.Subscription.Status): status is FailureStatus {

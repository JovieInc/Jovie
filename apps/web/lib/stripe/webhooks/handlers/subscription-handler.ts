...
import type { HandlerResult, SupportedEventType, WebhookContext } from '../types';
import { getCustomerId, getUserIdFromStripeCustomer, invalidateBillingCache } from '../utils';
...
export class SubscriptionHandler extends BaseSubscriptionHandler {
  ...
  async handle(context: WebhookContext): Promise<HandlerResult> {
    ...
    switch (event.type) {
      case 'customer.subscription.created':
        return this.handleCreated(subscription, stripeEventId, stripeEventTimestamp);

      case 'customer.subscription.updated':
        return this.handleUpdated(subscription, stripeEventId, stripeEventTimestamp);

      case 'customer.subscription.deleted':
        return this.handleDeleted(subscription, stripeEventId, stripeEventTimestamp);
  ...
  private async handleCreated(
    ...
    const userId = await this.extractUserId(subscription, 'customer.subscription.created');
  ...
  private async handleUpdated(
    ...
    const userId = await this.extractUserId(subscription, 'customer.subscription.updated');
  ...
  private async handleDeleted(
    ...
    const userId = await this.extractUserId(subscription, 'customer.subscription.deleted');
  ...
  private async extractUserId(
    ...
    if (!userId && typeof subscription.customer === 'string') {
      ...
      userId = (await getUserIdFromStripeCustomer(subscription.customer)) ?? undefined;
    }

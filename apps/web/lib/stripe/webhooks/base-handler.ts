...
import type { HandlerResult, SupportedEventType, WebhookHandler } from './types';
...
export abstract class BaseSubscriptionHandler implements WebhookHandler {
  ...
  protected async processSubscription(
    ...
    const { subscription, userId, stripeEventId, stripeEventTimestamp, eventType } =
      options;
  ...
  private async handleInactiveSubscription(
    ...
    const downgradeEventType: BillingAuditEventType =
      eventType === 'payment_failed' ? 'payment_failed' : 'subscription_downgraded';

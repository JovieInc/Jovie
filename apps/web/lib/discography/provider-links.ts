...
async function runIsrcLookups(
  ...
  if (providers.includes('apple_music') && !seenProviders.has('apple_music')) {
    lookupPromises.push(
      lookupAppleMusicByIsrc(isrc, options).then(result => {
        if (!result) return;
        links.push({
  ...
  if (providers.includes('deezer') && !seenProviders.has('deezer')) {
    lookupPromises.push(
      lookupDeezerByIsrc(isrc, { fetcher: options.fetcher }).then(result => {
        if (!result) return;
        links.push({
          provider: 'deezer',
...
export async function resolveProviderLinks(
  ...
  applyOverrides(options.overrides, links, seenProviders);

  if (track.isrc) {
    await runIsrcLookups(track.isrc, providers, seenProviders, links, {
      storefront,
      fetcher,
    });
  }

 ...
 * Combines the fixed tab bar height, device safe-area inset, and 24px of
 ...
 * Falls back to 12px when `env(safe-area-inset-bottom)` is zero (non-notched devices).
 *
 * Applied as: `pb-[max(env(safe-area-inset-bottom),12px)]`
 */
export const TAB_BAR_INTERNAL_SAFE_AREA_MIN_PX = 12 as const;

...
export function buildPitchDraftSystemPrompt(): string {
  return `You write release pitches in the artist's own voice. The pitch may be for a streaming curator, radio programmer, Sirius XM programmer, music supervisor, installation contact, editor, record label, or collaborator.
...
3. Proof or relevance: include only verified facts supplied in the input.
4. Clean ask: end with the lowest-friction next step for that destination.
...
export function buildPitchDraftUserPrompt(params: {
  ...
  const sections = [
    ...
    `Character limit: ${destination.characterLimit}`,
    ...buildArtistSection(input.artist),
    ...buildReleaseSection(input.release, input.tracks),
  ];

 ...
 * No emoji — GSM-7 stays one segment.
 */
...
/** Any merch order that means the artist already had a sale. */
...
/**
 * First paid merch sale for this artist → one celebratory SMS, forever.
...
export async function maybeNotifyFirstMerchSale(
  ...
  if (!phone) {
    logger.info('[merch] First-sale text skipped — no artist phone', {
      merchOrderId: input.merchOrderId,
    });

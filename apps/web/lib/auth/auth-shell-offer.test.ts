...
describe('auth offer reconciliation', () => {
  ...
  it.each(['max', 'team', 'enterprise'] as const)(
    ...
    plan => {
      ...
      const summary = resolveAuthOfferSummary({ handoff });
      expect(summary).toEqual({ title: 'Contact sales' });
      expect(JSON.stringify(summary)).not.toMatch(/trial|14-day|14 days|card/i);
      expect(

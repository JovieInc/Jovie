...
export const AUTH_SURFACE = {
  ...
  fieldShell:
    'flex w-full items-center gap-3 rounded-full border border-(--linear-app-frame-seam) bg-[color-mix(in_oklab,var(--linear-app-content-surface)_94%,var(--linear-bg-surface-0))] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] transition-[background-color,border-color,box-shadow] duration-subtle ease-subtle hover:border-default hover:bg-surface-0 focus-within:border-(--linear-border-focus) focus-within:bg-surface-0 focus-within:ring-2 focus-within:ring-(--linear-border-focus)/16',
  ...
  pillOption:
    'inline-flex min-h-8 items-center justify-center rounded-full border border-(--linear-app-frame-seam) bg-transparent px-3 text-[12px] font-[510] tracking-[-0.012em] text-secondary-token transition-[background-color,border-color,color,box-shadow] duration-subtle ease-subtle hover:border-default hover:bg-surface-1 hover:text-primary-token focus-visible:outline-none focus-visible:border-(--linear-border-focus) focus-visible:ring-2 focus-visible:ring-(--linear-border-focus)/16',
  pillOptionActive:
    'border-default bg-surface-1 text-primary-token shadow-[0_1px_1px_rgba(0,0,0,0.04)]',
  inlineAction:
    'inline-flex h-8 items-center justify-center gap-1.5 rounded-full border border-(--linear-app-frame-seam) bg-transparent px-3 text-[12px] font-[510] tracking-[-0.012em] text-secondary-token transition-[background-color,border-color,color,box-shadow] duration-subtle ease-subtle hover:border-default hover:bg-surface-1 hover:text-primary-token focus-visible:outline-none focus-visible:border-(--linear-border-focus) focus-visible:ring-2 focus-visible:ring-(--linear-border-focus)/16 disabled:pointer-events-none disabled:opacity-50',
  subtlePill:
...
export const AUTH_CLASSES = {
  ...
  oauthButtonMobile:
    'touch-manipulation select-none [-webkit-tap-highlight-color:transparent] active:scale-[0.98] transition-transform duration-subtle ease-subtle',
} as const;

export const RELEASE_COMMUNICATIONS_CONTRACT_VERSION =
  'release-communications/v1' as const;
...
export function mergeEventKey(
  event: Pick<
    VerifiedMergeEvent,
    'repository' | 'pullRequestNumber' | 'mergeSha'
  >
): string {
...
export function classifyMergeEvent(
  event: VerifiedMergeEvent
): Pick<DailyPostEntry, 'material' | 'audienceEligible'> {
  return {
    material: event.material ?? true,
    audienceEligible: event.audienceEligible ?? event.material ?? true,
  };
}

export class InMemoryReleaseCommunicationsAdapter
  implements ReleaseCommunicationsAdapter
{
  ...
  async ingest(event: VerifiedMergeEvent): Promise<DailyPost> {
    ...
    this.events.set(eventKey, entry);
    const nextPost = { ...post, entries: [...post.entries, entry] };
    this.posts.set(key, nextPost);
    return nextPost;
  }
    ...
    return (
      [...this.posts.values()].find(
        post =>
          ...
          post.localDate === input.localDate &&
  ...
  async dismissPost(input: { postId: string; userId: string }): Promise<void> {
    this.dismissals.add(`${input.postId}:${input.userId}`);
  }
  ...
  async isPostDismissed(input: {
    ...
    return this.dismissals.has(`${input.postId}:${input.userId}`);
  }

  private postFor(product: string, app: ReleaseApp, day: string): DailyPost {
    const post = this.posts.get(`${product}:${app}:${day}`);
    if (!post) throw new Error('deduplicated merge event has no daily post');
    return post;
  }
}

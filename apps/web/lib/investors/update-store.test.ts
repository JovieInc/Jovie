...
describe('investor update persistence boundaries', () => {
  ...
  it('validates source ownership against users.id after the Better Auth cutover', async () => {
    const source = await readFile(
      join(process.cwd(), 'lib/investors/update-store.ts'),
      'utf8'
    );

    expect(source).toContain('eq(users.id, input.sourceOwnerUserId)');

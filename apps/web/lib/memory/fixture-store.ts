...
export class MemoryFixtureStore implements MemoryStore {
  ...
  async findEntityByIdentity(
    ...
    const matchedIdentity = this.identities.find(
      row =>
        Boolean(this.scopedEntity(scope, row.entityId)) &&
  ...
  async findEntityByName(
    ...
    const alias = this.aliases.find(row => {
      const entity = this.scopedEntity(scope, row.entityId);
      return (
        entity?.type === type &&
        entity.status !== 'merged' &&
        normalizeMemoryName(row.alias) === normalized
      );
    });
    if (!alias) return null;
    return this.scopedEntity(scope, alias.entityId);
  }
  ...
  async getGraph(
    ...
    if (entityId && scopedEntities.length === 0) return emptyGraph();
    ...
    const assetMentions = this.assetMentions.filter(mention =>
      scopedEntityIds.has(mention.entityId)
    );
    const assetIds = new Set(assetMentions.map(mention => mention.assetId));
    const assets = this.assets.filter(
      asset =>
        isSameScope(asset, scope) && (!entityId || assetIds.has(asset.id))
    );
    const eventParticipants = this.eventParticipants.filter(participant =>
      scopedEntityIds.has(participant.entityId)
    );
    const eventIds = new Set(eventParticipants.map(row => row.eventId));
    const events = this.events.filter(
      event =>
        isSameScope(event, scope) && (!entityId || eventIds.has(event.id))
    );

...
export class MemoryCalendarLocationPhotoMatcher {
  ...
  async match(scope: MemoryScope): Promise<MemoryMatcherResult> {
    ...
    for (const event of graph.events) {
      ...
      const photoEntity = findAssetMentionEntity(
        graph,
        photo.id,
        entity => entity.id !== location.id
      );
      if (!photoEntity || photoEntity.id === location.id) continue;

      await this.store.createEdge(scope, {
        fromEntityId: location.id,
        toEntityId: photoEntity.id,
...
export class MemoryCatalogVoiceMemoMatcher {
  ...
  async match(scope: MemoryScope): Promise<MemoryMatcherResult> {
    ...
    for (const voiceMemo of graph.assets.filter(
      ...
      if (!song) continue;
      const recording = findAssetMentionEntity(
        graph,
        voiceMemo.id,
...
function findAssetMentionEntity(
  graph: MemoryGraphSnapshot,
  assetId: string,
  predicate: (entity: MemoryEntity) => boolean
): MemoryEntity | null {
  for (const mention of graph.assetMentions.filter(
    row => row.assetId === assetId
  )) {
    const entity = graph.entities.find(row => row.id === mention.entityId);

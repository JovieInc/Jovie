...
export class MemoryIdentityResolver {
    ...
    if (!created && candidate.metadata) {
      await this.store.updateEntity(scope, entity.id, {
        metadata: mergeMetadata(
          entity.metadata,
          buildEvidenceMetadata(candidate.evidence, candidate.metadata)
        ),
      });
    }
  ...
  private async addAliases(
    ...
    const sourceIds = evidenceSourceIds(candidate.evidence);
    for (const alias of aliases) {
      for (const sourceId of sourceIds) {
        await this.store.addEntityAlias(scope, entityId, alias, sourceId);
      }
    }
  }

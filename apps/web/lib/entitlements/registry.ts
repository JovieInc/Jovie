...
/**
 * Resolve a raw plan string to the chat-usage display/quota tier.
 * Unlike {@link getEntitlements}, this maps legacy names
 * ('founding' -> 'pro', 'growth' -> 'max') and returns 'free' for
 * null, undefined, or unknown plans. Active `trial` is preserved so
 * usage UI can show the 50/week trial quota instead of Free copy.
 */
export function resolveChatUsagePlan(plan: string | null | undefined): PlanId {

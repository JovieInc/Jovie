...
export function LabelPill({ children, className }: LabelPillProps) {
  return (
    <span
      className={cn(
        'inline-flex items-center h-5 px-1.5 rounded text-3xs font-caption text-tertiary-token bg-(--surface-1)/40 border border-(--linear-app-shell-border)/50 whitespace-nowrap',
        className
      )}
    >
      {children}
    ...
    >
      ...
      {rest.length > 0 && (
        <>
          <span
            aria-hidden='true'
            className='inline-flex items-center h-5 px-1.5 rounded text-3xs font-caption text-quaternary-token bg-(--surface-1)/60 border border-(--linear-app-shell-border)/50 group-hover/labels:hidden'
          >
            +{rest.length}
          </span>
          {rest.map(l => (

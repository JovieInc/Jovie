...
export function PickerLink({ href, label, className }: PickerLinkProps) {
  return (
    <a
      href={href}
      className={cn(
        'flex items-center justify-between h-6 px-1.5 rounded text-2xs text-secondary-token hover:bg-surface-1 hover:text-primary-token focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-token transition-colors duration-subtle ease-subtle',
        className
      )}
    >
      <span>{label}</span>

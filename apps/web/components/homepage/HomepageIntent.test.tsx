...
describe('HomepageIntent', () => {
  ...
  it('1. renders headline, subhead, input, and the refreshed pill set', () => {
    ...
    expect(
      screen.getByRole('heading', {
        name: 'Drop more music.\nCrush every release.',
      })
    ).toBeTruthy();
    expect(
      screen.getByText('One system to make every release count, every time.')
    ).toBeTruthy();
  ...
  it('6. submit calls router.push with /start and intent id', () => {
    render(<HomepageIntent />);
    fireEvent.change(getInput(), { target: { value: 'anything' } });
    fireEvent.click(getSubmit());
    expect(mockPush).toHaveBeenCalledTimes(1);
    const target = mockPush.mock.calls[0][0] as string;
    expect(target).toContain('/start?intent_id=');
  });

...
export function ArtistCarousel({ artists }: ArtistCarouselProps) {
  ...
  return (
    <section className='relative w-full max-w-full overflow-hidden py-16 bg-page border-y border-subtle'>
        ...
        <div className='flex space-x-6 px-4 sm:px-6 lg:px-8 py-8 min-w-max'>
          {artists.map(artist => (
            <Link
              key={artist.id}
              href={`/${artist.handle}`}
              className={`group shrink-0 ${prefersReducedMotion ? '' : 'transition-[transform,scale] duration-300 hover:scale-110'}`}
            >

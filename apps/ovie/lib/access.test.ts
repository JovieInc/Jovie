...
describe('public entry boundary', () => {
  ...
  it.each(['/signin', '/_next/image', '/favicon.ico', '/api/auth/get-session'])(
    'never exempts server actions at %s',
    pathname => {
...
describe('workflow migration boundary', () => {
  it.each([
    ...
    'never treats SDK callback metadata as public authorization at %s',
    pathname => {

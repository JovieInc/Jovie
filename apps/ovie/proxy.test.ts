...
describe('private Ovie request gate', () => {
  it.each(protectedRequests)(
    'denies anonymous %s %s before reaching handlers',
  ...
  it.each(protectedRequests)(
    'denies a signed-in nonadmin %s %s',
    async (path, method, headers) => {
  ...
  it.each([...protectedRequests, ['/app/ov/ops', 'GET', {}] as const])(
    'passes authorized admin %s %s to existing handler authorization',
    async (path, method, headers) => {
      mocks.getSession.mockResolvedValue({ user: { id: 'ba-user-id' } });
  ...
  it.each(['GET', 'HEAD'])(
    'redirects anonymous page %s to same-origin signin preserving the deep link',
    async method => {
  ...
  it.each(['getSession', 'getAppUser', 'isAdmin'] as const)(
    'fails closed with sanitized 503 if %s throws',
    async dependency => {
      mocks.getSession.mockResolvedValue({ user: { id: 'ba-user-id' } });
  ...
  it.each([
    ...
    'allows public entry %s %s while auth is unavailable',
    async (path, method) => {

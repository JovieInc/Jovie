...
async function openSignIn(apiBaseUrl: string) {
  const signInUrl = new URL('/sign-in', apiBaseUrl);
  window.open(signInUrl.toString(), '_blank');
}
...
function renderSignedOut() {
  ...
  footer.append(
    createButton('Sign In', 'primary', async () => {
      const apiBaseUrl = await resolveApiBaseUrl(state.currentTabUrl);
      await openSignIn(apiBaseUrl);
    })
  );

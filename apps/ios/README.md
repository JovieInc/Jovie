...
2. Generate local iOS config:
   ...
   The local generator writes `API_BASE_URL` and `WEB_BASE_URL` as
   `http://localhost:3100` unless you override them. Native authentication is
   Better Auth PKCE and does not require a client-side provider key.
...
4. Run tests:
   ...
   Live Better Auth tests are skipped by default. The test wrapper forwards the
   live-auth environment into XCTest via
...
Release CI uses these GitHub secrets: `APPLE_API_KEY`, `APPLE_API_KEY_ID`, `APPLE_API_ISSUER`, `APPLE_TEAM_ID`, `MATCH_GIT_URL`, `MATCH_PASSWORD`, and one of `MATCH_GIT_PRIVATE_KEY` or `MATCH_GIT_BASIC_AUTHORIZATION`.
...
- Real-device and TestFlight auth must use one canonical HTTPS environment for
  ...
  tunnel, set both values to the same tunnel origin.
- iOS auth uses the custom-scheme callback above; no provider SDK or provider
  webcredentials entitlement is linked into the app.

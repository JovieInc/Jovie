...
export function verifyFounderPrincipal(
  ...
  if (
    ...
    !/^[A-Za-z0-9_-]{43}$/u.test(principalHash)
  )
    return false;
...
/** Only the durable admission creator may dispatch. Ambiguous sends require result reconciliation. */
    ...
    try {
      ...
      if (existing) {
        ...
        if (
          ...
          parsedAccepted.data.principalHash === input.principalHash &&
          parsedAccepted.data.deploymentId === input.deploymentId
        )
          return json(
        ...
        // A reservation is the durable send fence, including after UTC rollover.
        // Reconciliation may discover its result; ingress must never send again.
      ...
      if (
        ...
        const persistedIntent = await deps.read(intentPath);
        if (persistedIntent && persistedIntent.bodySHA256 !== bodySHA256)
          return json({ ok: false, code: 'event_conflict' }, 409);
        return json({ ok: false, code: 'dispatch_unknown' }, 503);
      }
      ...
      if (admissionWrite !== 'created') {
        ...
        // An existing admission may already have reached Eve. Even a native
...
export async function readConversationResult(input: {
  ...
  if (existing) {
    ...
    return parsed.success && matchesBinding(parsed.data)
  ...
  if (!parsedAccepted.success && input.recoverSession) {
    ...
    if (admission.success && matchesBinding(admission.data)) {
  ...
  if (!parsedAccepted.success || !matchesBinding(parsedAccepted.data))
    return json({ ok: false, code: 'accepted_turn_unavailable' }, 503);

...
Net: full-send on decomposition of #12498, scoped to "`eve` as the agent adapter, behind a flag," not
...
(Vercel Workflows). So: adopting `eve`'s **runtime layer** is additive; adopting `eve`'s **runtime +
...
- `apps/web/lib/chat/run.ts` (462 LOC) — `executeChatTurn`: builds the system prompt, converts
  ...
  `stepCountIs` + `prepareStep` + `ToolSet`). The bespoke agent loop is mostly the SDK's own agent
...
- `eve` replaces the **`AgentHarness` box only**, i.e. it is the concrete adapter in place of the
  `OpenAIAgentsAdapter` stub. This is consistent with #9871's interface-first design, which left the
  adapter deliberately swappable. The founder pivot from "OpenAI Agents SDK" to "`eve`" is a change of

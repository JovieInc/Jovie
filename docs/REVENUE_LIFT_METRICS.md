...
|---|---|---|---|
| 1 | **Dollarization weights undefined.** `streaming_value_weight` and `fan_capture_ltv_weight` live nowhere; lift is only raw click/fan counts. | A single constants module (e.g. `apps/web/lib/metrics/revenue-lift-weights.ts`) exporting both weights with cited assumptions + a unit test; a query returns dollarized `revenue_lift` per run. | Required |
| 2 | **No active-artist cohort denominator + baseline.** Can't divide lift by artists or compute lift %. | A query/view returning `active_artists_in_window` and a pre-automation revenue baseline per artist so IRPAA and lift % are computable. | Required |
| 3 | **No canonical IRPAA rollup.** `sumArtistAutomationAttributedRevenue` returns raw deltas for one artist; nothing weights them or averages across the cohort. | A `getIRPAA(window)` query joining gaps 1+2 over `workflow_run_outcomes`, with a deterministic test. | Required |
| 4 | **Fan-capture LTV is ungrounded.** Only a subscription-MRR proxy exists (`computeLtv`, `apps/web/lib/admin/funnel-metrics.ts:495`); no fan LTV weighted from streaming click-throughs. | A documented fan-capture LTV proxy (weight feeding gap 1) with a 30-day validation column comparing proxy vs. realized GMV/tips. | Required |

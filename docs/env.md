...
Artist first-sale SMS live switch (JOV-5207). Unset or `false` is dry-run:
the first paid merch order is still claimed, but Twilio is not called.
Set to `true` only after artist phone coverage is ready. Actual provider
POSTs still require `OUTBOUND_SMS_ENABLED`.

...
- **First merch-sale text (JOV-5207):** artist-facing, once forever per creator. Hook is Stripe merch `checkout.session.completed` (`handleMerchCheckoutCompleted`), not Printful fulfillment. Copy lives in `lib/merch/first-sale-text.ts` (`FIRST_SALE_TEXT_COPY`). Sends through `sendNotification` SMS. Live send requires `FIRST_SALE_TEXT_LIVE=true` (default dry-run) plus `OUTBOUND_SMS_ENABLED`. Idempotency is a CAS write to `creator_profiles.settings.firstSaleText`. Destination is the owning user's Better Auth `phoneNumber` when present — no phone means claim + skip, not a fake send.

...
2. `preflight` requires current account, auth, thread scopes, model, balance,
   auto-recharge-off, per-query/period caps, useful outcome, destination,
...
4. Observations must preserve thread/key identity and increase monotonically.
...
8. Transport loss, stale state, or 5xx gets one reconciliation pass. A single
   same-key retry is allowed only after provider-proven absence or documented
   idempotent replay.
9. Useful terminal success requires content, usage, and cost receipts. External
   delivery is a separate evidence tier.
...
monotonic observations, crash/reconnect reservation persistence, matching
provider-result receipts, one reconciliation, and one safe retry. Synthetic

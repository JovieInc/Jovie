...
| **Agent PR Verify Ready** | `agent-pr-verify-ready.yml` | `pull_request_target` (opened/synchronized/reopened/edited); trusted `issue_comment` (created) | Produces advisory `Agent Evidence Ready` per head; metadata events redraft agent pushes/retargets, while only an actor-bound created comment runs isolated head checks and can publish success. JOV-3461 owns required-check activation and rollout. | Per-PR | **Keep** |

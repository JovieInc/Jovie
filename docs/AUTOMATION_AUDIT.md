...
| **Merge Queue Auto-Enroll** | `merge-queue-autoenroll.yml` | PR/CI/main events, exact ownerless intent, 15-minute maintenance heartbeat, and manual | Revalidates the exact current head, performs bounded lost-event/receipt recovery, refreshes clean stale agent heads, escalates conflicts, and enrolls only through GitHub's native merge queue | Canonical queue and generic stale-head remediation controller; live operation requires a successful checked-in workflow run | **Keep enabled first** — sole native queue enrollment owner; never infer live health from source alone |
| **Auto-Ready Agent Drafts** | `auto-ready-agent-drafts.yml` | source PR/CI retry events + manual; no schedule | On the exact trusted current head, pairs ready-for-review with native auto-merge intent immediately; pending source checks remain GitHub merge gates | First controller enabled during drain | **Keep enabled first** — canonical paired-promotion owner; never wait for green or create a second CI flight |

...
|------|----------|-----------|----------|---------|----------------|-----------|
| `/api/cron/frequent` | `*/15 * * * *` | Every 15 min | Yes | Orchestrates existing sub-jobs plus an hourly Redis write/read operability canary with stable quota-failure reporting | **Keep** | Core heartbeat; consolidation avoids a new schedule. Redis canary adds 2 base commands/hour (1,488/month; Global write replication may raise the provider total) and detects write-blocking quotas that `PING` misses. |
...
| **Auto-Ready Agent Drafts** | `auto-ready-agent-drafts.yml` | writer-proof: manual dispatch only. green-source: `workflow_run` (CI / Fork PR Gate / PR Size Guard success) + successful GitHub Actions `check_suite` + targeted dispatch; no schedule | Writer-proof recovery still pairs ready + native auto-merge from an exact-head `jovie-writer-pr-proof/v1` receipt. Green-source undrafts a main draft only when PR Ready + required checks are SUCCESS and mergeability is CLEAN, then stops — Auto-Enroll owns queue admission. Never mutates HOLD #17156 or Tim-hold / machine hold labels | First controller enabled during drain | **Keep enabled first** — restore the July green-source wake so drafts can enter the ready pool; writer-proof stays manual and never waits for green |
...
| **M2 Revenue-Path Canary** | `m2-revenue-path-canary.yml` | Daily `37 6 * * *` UTC + Production Controller `workflow_run` + manual / `workflow_call` | Signed-out → claim → $199 Pro checkout → activation against prod or staging that mirrors checkout. Timestamped receipt; Slack + Linear with repro on red. No Stripe session creation. | Daily + post-controller | **Keep** — money-path liveness, distinct from Canary Health Gate |
...
| **Linear Sync on Merge** | `linear-sync-on-merge.yml` | `pull_request` (closed) | Extracts Linear issue marker from merged PR. Transitions the issue to Done and posts the merge SHA only when every linked PR is merged or closed and the issue is not a commissioning or parent issue. Otherwise comments and leaves it open | Per-merge | **Keep** — closes the Linear state loop for merged work without closing stacked or parent issues early |
...
| **Agent PR Verify Ready** | retired (workflow disabled and removed) | none | Duplicated source-PR verification and promoted drafts from stale event runs | none | **Retired** — the current-head Auto-Ready controller pairs ready + native auto-merge before the sole source CI flight completes |

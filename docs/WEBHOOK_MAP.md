...
| `/api/webhooks/linear` | Linear | HMAC-SHA256 (`linear-signature` + `LINEAR_WEBHOOK_SECRET`, timing-safe) | CodeRabbit plan-ready comment | Deduplication via Redis. Fires GitHub `repository_dispatch` (`linear_plan_ready`) to trigger CI automation; issue pickup is owned by upstream OpenAI Symphony polling Linear. |

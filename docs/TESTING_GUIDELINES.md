...
| `.github/workflows/test-coverage-audit.yml` | Nightly cron at 18:30 UTC, after the deterministic nightly lanes. Runs `test:coverage:diff` against the committed snapshot, then commits the heatmap when values change. Slack on failure. Merge-queue unit shards stay coverage-off. |
...
**Layout shift & visual stability testing is mandatory risk-based coverage.** Prevent unexpected or uninitiated instability, while allowing geometry that is the bounded, local, and deterministic result of explicit disclosure or navigation. Use Playwright `boundingBox()` or `getBoundingClientRect` helpers to prove both: no initial or system-initiated movement, and exact geometry ownership after a user action. CLS excludes layout shifts shortly after qualifying user input, so a green Web Vitals value is necessary but insufficient; source guards must also require bounded, local, and deterministic state ownership. Async/loading/error/content changes reserve space or use overlays when reflow is not semantic. Interactive disclosures collapse fully and may move only their own declared flow.

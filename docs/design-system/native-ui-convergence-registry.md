...
| `ios.onboarding` | Native profile completion; `NeedsOnboardingView` inside `AppShellView` | `apps/ios/Jovie/Features/NeedsOnboarding/NeedsOnboardingView.swift:L3-L143`; `apps/ios/Jovie/App/RootView.swift:L76-L128` | Prefilled display name and handle, validation, saving, recoverable error; chat/audience/library/calendar/inbox disabled in this route | Padded full-width form; core completion remains native and has no generic web fallback | `verified-source`; current coverage in `feature-status.csv:L6`. |
...
| --- | --- | --- | --- |
| Auth/onboarding | Browser auth followed by native profile completion, `AuthScreen.swift:L27-L207`, `NeedsOnboardingView.swift:L3-L143` | Auth routes, the mobile profile-completion endpoint, and public profile are separate Next route trees | `verified-contract`; authentication hands off to a native completion form backed by the mobile API. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Launch/auth/onboarding | Yes | Auth loading; profile saving | Native profile fields require display name and handle | Auth error; recoverable profile-save error; app route error | AppState retains stale profile on transport failure, `AppState.swift:L123-L221` | Disabled/loading controls; native retry preserves entered fields | Profile completion must resolve to `.ready`; no generic web escape is available. |

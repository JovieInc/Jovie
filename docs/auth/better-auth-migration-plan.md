...
6. `secondaryStorage` returns strings + contract test; session/revocation `delete` fails closed (fail-open revocation otherwise), while database-authoritative `verification:` keys bypass Redis. PKCE routing state and native exchange records share that Postgres authority rather than making Redis an auth availability dependency.

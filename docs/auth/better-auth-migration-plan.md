...
- **Native handoff:** transport unchanged (deep links, PKCE record, handoff window, Keychain, cookie jar). PKCE routing state and native exchange codes use Better Auth's Postgres-backed verification adapter, so Redis quota exhaustion cannot block sign-in; both remain short-lived and atomically consumed. `/auth/callback` mints an OTT from the live browser session; `/api/auth/native/exchange` — iOS: server verifies OTT and returns a **fresh** session token (`internalAdapter.createSession`, independent of the browser session) for Keychain+bearer; Electron: returns the OTT, the `native-complete` page POSTs it to the built-in `/api/auth/one-time-token/verify` which sets the session cookie. iOS refreshes token/expiry from the bearer plugin's `set-auth-token` response header.
- **Redis:** `apps/web/lib/auth/secondary-storage.ts` over `getRedis()` — 500ms timeout race, returns strings (guards Upstash `automaticDeserialization`), Sentry-warn; `get`/`set` best-effort. Session and revocation `delete` calls **fail closed** when Redis is reachable (retry/escalate). When the client is missing (quota circuit, unconfigured, init failure), delete/getAndDelete skip and warn so Postgres-backed sign-out stays available. Better Auth verification records use `verification.storeInDatabase: true`; their `verification:` secondary-storage keys bypass Redis so Postgres remains the single atomic authority and Redis exhaustion cannot invalidate an already-consumed OTP. In-memory fallback is local/test only; production rate-limit storage is Redis-or-DB, never in-memory. Postgres remains the durable session store.
...
6. `secondaryStorage` returns strings + contract test; session/revocation `delete` fails closed when Redis is reachable (fail-open revocation otherwise) and degrades when the client is missing, while database-authoritative `verification:` keys bypass Redis. PKCE routing state and native exchange records share that Postgres authority rather than making Redis an auth availability dependency.
...
| Fail-open revocation | Non-verification `delete` fails closed when Redis is reachable; a missing client skips Redis so Postgres revocation can complete |

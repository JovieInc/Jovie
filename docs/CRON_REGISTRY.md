...
| 12 | youtubeLibraryRefresh | Every invocation | Re-syncs at most one connected YouTube channel stale >24h via `runConnectedYouTubeRefreshes`; refreshes OAuth tokens, preserves the last good import on failure, and reconciles approved catalog collaborators. |

...
Library optimization uses the existing telemetry surfaces only. Do not add a
parallel analytics stack.

| Surface | Canonical owner |
| --- | --- |
| analytics | `apps/web/lib/analytics/metrics.ts` |
| model-experiment | `apps/web/lib/db/schema/model-experiments.ts` |
| audience-event | `apps/web/lib/audience/record-audience-event.ts` |
| youtube-experiment | `apps/web/lib/youtube-library/thumbnail-experiments.ts` |
| release-to-revenue | `apps/web/lib/release-to-revenue/gmv-attribution.ts` |

Library experiment rows (`optimization_experiments` in
`apps/web/lib/db/schema/library-content-graph.ts`) are the subject/variant
ledger for a selected content card. They must join those surfaces for
exposure, outcome, attribution, and writeback. A global score is forbidden.

### Optimization contract

kind: product
variantIdentity: `library-content-card:{kind}:{canonicalId}:{experimentId}:{variantKey}` from `optimization_experiments.variants`, with YouTube thumbnail arms keyed by `youtubeThumbnailVersions.experimentId`
exposure: audience-event impression or click on the selected Library content card or public destination; YouTube packaging experiments record exposure through locked measurement windows
outcome: artist-business-outcome via release-to-revenue attributed GMV or paid conversion for the same content/release context; analytics canonical metrics remain intermediate signals only
attribution: release-to-revenue plus analytics session; YouTube and Library decisions persist on `optimization_experiments.decisionEvidence`
contextDimensions: platform, medium-or-channel, country-or-locale, genre-or-cohort, artist-plus-career-era-or-lifecycle, content-variant, consented-audience-segment
hypothesis: A track-first Library card with evidence-backed presence, destinations, and packaging variants increases paid conversion and durable fan value without selling licenses or inventing metrics
primaryMetric: artist-business-outcome: paid conversion or attributed GMV per eligible exposure
guardrails: complaint, trust, brand
privacy: first-party consented behavior only; no sensitive demographic inference or cross-platform identity stitching
optimizerOwner: Library product / Symphony
cadence: weekly decision with writeback after a locked measurement window
decisionWriteback: model-experiment promotion receipt plus `optimization_experiments.winnerVariantKey` and explicit `promoteThumbnailWinner`
...
disconnected stats remain fail-closed guardrails around this contract.

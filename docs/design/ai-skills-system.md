...
- [x] `promotion.voice-clone` (YouTube URL → training data → 11 Labs IVC) — partial v1 landed gh-9807 (HOT ZONE: lib/ai/tools/voice-clone.ts + test; uses IVC endpoint, consent gate, yt-dlp guidance; no schema/deps). Enables voice-promo.
- [ ] `promotion.voice-promo` — ElevenLabs voice cloning integration (builds on voice-clone voice_id)

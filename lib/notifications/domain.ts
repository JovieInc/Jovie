...
export const subscribeToNotificationsDomain = async (
...
): Promise<NotificationDomainResponse<NotificationSubscribeResponse>> => {
  ...
  try {
    ...
    if (!artistProfile) {
      ...
      return buildErrorResponse(404, 'Artist not found', 'not_found');
    }
    ...
    if (dynamicEnabled && normalizedEmail) {
      const ip = getForwardedIp(context.headers);
...
export const unsubscribeFromNotificationsDomain = async (
  ...
  try {
    ...
    const normalizedPhone = phone ? normalizeSubscriptionPhone(phone) : null;

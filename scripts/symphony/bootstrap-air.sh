...
DOPPLER_CONFIG="dev"
INSTALL_HELPER="${REPO_ROOT}/scripts/symphony/lib/install-launchd-artifacts.sh"

# shellcheck source=scripts/symphony/lib/install-launchd-artifacts.sh
...
if [[ ! "$NODE_VERSION" =~ ^v22\.([0-9]+)\.([0-9]+) ]]; then
  die "Node 22.x required, found $NODE_VERSION. Run: nvm use 22 && corepack prepare pnpm@9.15.9 --activate"
fi
NODE_MINOR="${BASH_REMATCH[1]}"
NODE_PATCH="${BASH_REMATCH[2]}"
if ((10#$NODE_MINOR < 23 || (10#$NODE_MINOR == 23 && 10#$NODE_PATCH < 1))); then
  die "Node >=22.23.2 required, found $NODE_VERSION. Run: nvm use 22 && corepack prepare pnpm@9.15.9 --activate"
fi
...
tmpl = Path(repo_root, "scripts/symphony/config.air.template.yaml").read_text()
...
hermes_stage_artifact \
  "${REPO_ROOT}/scripts/symphony/shipper-gated-entrypoint.py" \
...
for tmpl in "${REPO_ROOT}/scripts/symphony/launchd/"*.plist.template; do
  [[ -f "$tmpl" ]] || continue

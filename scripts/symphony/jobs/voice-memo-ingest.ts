 ...
 *
 * Usage:
 *   pnpm tsx scripts/symphony/jobs/voice-memo-ingest.ts          # scan dir
 *   pnpm tsx scripts/symphony/jobs/voice-memo-ingest.ts --file /path/x.m4a
 */
...
async function fileIssueFromSpan(args: {
  ...
  if (filed.queued) {
    logJobEvent({
      job: JOB,
      event: 'issue_queued_keeping_memo_for_retry',
      file: basename(args.memoFile),
    });
  }
  return null;
}

...
readonly UNIT_TEMPLATE_SOURCE="${SOURCE_ROOT}/scripts/symphony/systemd/gem-ship-hud.service.template"
readonly HUD_SOURCE="${SOURCE_ROOT}/scripts/symphony/gem-checkin-hud.py"
readonly HUD_WRAPPER="${SOURCE_ROOT}/scripts/symphony/gem-checkin-tty1.sh"
...
SOURCE_DIRTY="false"
if ! git -C "${SOURCE_ROOT}" diff --quiet -- "${HUD_SOURCE}" "${HUD_WRAPPER}" "${UNIT_TEMPLATE_SOURCE}" scripts/symphony/install-gem-ship-hud.sh; then
  SOURCE_DIRTY="true"
fi
export \
  ATTESTATION_TARGET \

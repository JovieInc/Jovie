...
Templates for the Jovie-owned launchd unit files installed by `scripts/symphony/bootstrap-air.sh`.
...
Coder/shipping loops run on Houston, not Hermes-Air. Pro-only templates live in `pro/` and are installed by `scripts/symphony/bootstrap-pro-launchd.sh` (not `bootstrap-air.sh`).

| File | Schedule | Purpose |
|---|---|---|
| `pro/co.jovie.hermes.cron-codex-kanban-ship.plist.template` | every 15 min | Launch `scripts/symphony/ship-loop.sh` → `~/.hermes/scripts/codex-kanban-ship.py` (PAUSE + gbrain gated) |

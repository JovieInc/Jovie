...
RUNNER.chmod(0o700)
RUNTIME_BINARY = ROOT / "symphony-runtime-fixture"
RUNTIME_BINARY.write_bytes(b"synthetic-beam-runtime-image")
SOURCE = Path(__file__).resolve().parents[3]
RUNTIME = {"schema": contract.V2_RUNTIME_IDENTITY_SCHEMA, "service": contract.V2_OFFICIAL_RUNTIME_SERVICE,
    "sourceRevision": subprocess.check_output(["git", "-C", str(SOURCE), "rev-parse", "HEAD"], text=True).strip(),
    "binarySha256": trust.digest(RUNTIME_BINARY),
...
def proof(now, name="1"):
    ...
    p = {"runtimeGeneration": GENERATION, "codexSha256": trust.digest(RUNNER), "accountStateSha256": row["accountStateSha256"], "schema": contract.V2_PROOF_SCHEMA, "producer": contract.V2_PROOF_SOURCE, "agentProfile": "coder",
...
def refresh(now):
    write_private(CONTEXT, {"runtime": RUNTIME, "sourceRoot": str(SOURCE),
        "binaryPath": str(RUNTIME_BINARY), "codexPath": str(RUNNER), "codexSha256": trust.digest(RUNNER), "workflowPath": str(SOURCE / "scripts/symphony/WORKFLOW.md"),
...
def evidence(target=4, observed_at=None):
    ...
    refresh(now)
    return {"schema": contract.V2_CAPACITY_SCHEMA, "source": contract.V2_CAPACITY_SOURCE,

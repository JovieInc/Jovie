...
class HyperagentLifecycleTests(unittest.TestCase):
    ...
    def test_preflight_accepts_only_a_complete_live_selected_route(self):
        ...
        for field in (
            ...
            "credits_expire_at", "issue_id", "lease_id", "expected_pr_repository",
            "required_runtime",
    ...
    def test_observation_age_and_transport_loss_reconcile_the_original(self):
        ...
        self.assertEqual(
            lifecycle.plan_resolution(classified)["action"],
            "reconcile_issue_lifecycle_once",
        )
    ...
    def test_provider_failures_have_bounded_distinct_actions(self):
        ...
        for code, action in expected.items():
            ...
            self.assertEqual(lifecycle.plan_resolution(classified)["action"], planned_action)
    ...
    def test_actual_approval_requires_structured_identity_not_approval_words(self):
        ...
        actual = lifecycle.classify_observation(self.observation(interaction=self.approval()), self.now)
    ...
    def test_approval_resolution_requires_exact_authority_scope_and_surface(self):
        classified = lifecycle.classify_observation(self.observation(interaction=self.approval()), self.now)
        ...
        mismatched = lifecycle.plan_resolution(classified, self.authority(destination="other"), ["approvals:write"])
        self.assertFalse(mismatched["execute"])
    ...
    def test_web_only_approval_needs_an_attended_exactly_authorized_fallback(self):
        ...
        classified = lifecycle.classify_observation(self.observation(interaction=approval), self.now)
        self.assertFalse(lifecycle.plan_resolution(classified, self.authority())["execute"])
    ...
    def test_input_memory_and_domain_are_not_collapsed_into_approval(self):
        ...
        input_state = lifecycle.classify_observation(self.observation(interaction=input_interaction), self.now)
        ...
        for authority, scopes in (
            ({**self.authority(), "input_authorized": True, "prompt_sha256": "b" * 64,
              "response_sha256": "c" * 64, "thread_id": "other"}, ["threads:write"]),
            ({**self.authority(), "input_authorized": True, "prompt_sha256": "b" * 64,
              "response_sha256": "c" * 64}, ["threads:read"]),
        ):
            self.assertFalse(lifecycle.plan_resolution(input_state, authority, scopes)["execute"])
            self.assertEqual(
    ...
    def test_terminal_failure_decline_cancel_and_unknown_remain_distinct(self):
        expected = {
            "failed": "remote_failed", "declined": "remote_declined",
            "cancelled": "remote_cancelled",
        }
        for terminal, state in expected.items():
            result = lifecycle.classify_observation(self.observation(is_running=False, terminal_state=terminal), self.now)
            self.assertEqual(result["state"], state)
            self.assertEqual(
                lifecycle.plan_resolution(result)["action"],
                "reconcile_issue_lifecycle_once",
            )
    ...
    def test_cli_preflight_and_classification_are_machine_readable(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            classification.write_text(json.dumps({
                ...
                "job": {"thread_id": "thread-1", "idempotency_key": "job-1"},
            }))
            authority.write_text(json.dumps(self.authority()))
            planned = subprocess.run(
                [
                    "python3", str(SOURCE), "plan", "--classification", str(classification),
                    "--authority", str(authority), "--oauth-scope", "approvals:write",
                ],
                text=True, capture_output=True, check=True,
            )
            self.assertEqual(json.loads(planned.stdout)["action"], "resolve_approval_once")

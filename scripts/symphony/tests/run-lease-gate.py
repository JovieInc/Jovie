#!/usr/bin/env python3
"""Run lease behavior and process regressions with coverage of the changed transaction."""
import ast
import dis
import json
...
SUITE = ROOT / "scripts/symphony/tests/symphony-lease-guard.test.py"
TARGETS = {
    "symphony-lease-guard": {"_state_transaction", "_save_state", "lease_decision", "check", "_existing_repair_preflight"},
}
...
for name, selected in TARGETS.items():
    ...
    if percent < 95:
        status = 1
print(json.dumps({"selectors": [str(SUITE.relative_to(ROOT)), str(repair_suite.relative_to(ROOT))], "coverage": report}, indent=2))

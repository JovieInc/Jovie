...
def _consumer_workspace_env():
    """Give consumer receipt writes an owned, disposable GEM_WORKSPACE."""
    with tempfile.TemporaryDirectory() as directory:
        workspace = pathlib.Path(directory)
        workspace.chmod(0o700)
        with mock.patch.dict(
            CYCLE.os.environ, {"GEM_WORKSPACE": str(workspace)}, clear=False
        ):
            yield workspace
...
class FleetControllerInstallerContractTests(unittest.TestCase):
    ...
    def test_codex_profile_adopts_router_and_five_agents_from_codex_out(self):
        ...
        front, body = installed.split("---", 2)[1:]
        ...
        self.assertIn("- no-symphony", front)
        self.assertIn("- billing", front)
        self.assertIn("- vercel", front)
        self.assertIn("protected-intake-check", front)
        self.assertIn("#17453", body)
        self.assertIn("#17156", body)

...
# Measure both the existing provider filter and the canonical alias grouping.
expected = set()
for node in ast.parse(source).body:
    if isinstance(node, ast.FunctionDef) and node.name in {"codex_account", "account_groups"}:
        code = next(value for value in compile(source, filename, "exec").co_consts if hasattr(value, "co_name") and value.co_name == node.name)
        expected.update(line for _, line in dis.findlinestarts(code) if node.lineno < line <= node.end_lineno)
...
print(json.dumps({"selector": str(suite.relative_to(ROOT)), "account_filter_and_identity_groups": {"executed": len(expected - set(missing)), "statements": len(expected), "missing": missing}}, indent=2))

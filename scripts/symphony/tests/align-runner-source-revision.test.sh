#!/usr/bin/env bash
# Refuse dirty/mismatch without ALIGN_ALLOW_CHECKOUT; align + idempotent with it.
set -euo pipefail
...
printf 'a\n' >"${TMP}/cfg/f"; git -C "${TMP}/cfg" add f; git -C "${TMP}/cfg" commit -qm first
SHA1="$(git -C "${TMP}/cfg" rev-parse HEAD)"
printf 'b\n' >"${TMP}/cfg/f"; git -C "${TMP}/cfg" add f; git -C "${TMP}/cfg" commit -qm second

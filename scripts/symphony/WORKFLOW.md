...
Hyperagent remains one provider behind that router, never a parallel queue. Before `create_thread`, run `scripts/symphony/hyperagent/lifecycle.py preflight` against a fresh sanitized envelope and stop unless it returns `PROCEED`; then call `LifecycleJournal.reserve_dispatch` and create only when its atomic payer/account/budget-period reservation returns `execute_create: true`. The budget snapshot, immutable full dispatch identity (including issue, lease, PR repository, and required runtime), and unpredictable attempt identity must be HMAC-authenticated before creation. An ambiguous create retains exposure until signed reconciliation finds the existing thread or proves provider absence; only the latter permits one exact-identity retry. `register_dispatch` cannot backfill an unreserved thread. Observe only the authenticated bound thread with bounded backoff, and record PR, merge, and required-runtime evidence separately through `observe_delivery`. Obtain the allowed next step from the Python `plan_resolution(...)` API. The pure plan never authorizes direct execution: reserve a mutation atomically with `reserve_action_once`, perform it only when that returns `execute: true`, and record its HMAC-authenticated result with `record_action_result`. Never infer an approval from prose. Keep actual approval, ordinary required input, memory approve/reject, sandbox-domain approval, stale status, transport loss, provider failure, remote completion, open PR, merge, and required-runtime proof distinct. Recover an existing PR through the native merge controller instead of redispatching its issue. `reserve_retry_once` may reserve attempt 2 only after trusted revision-current evidence proves the remote job absent or idempotently replayable and no PR exists; it repeats the aggregate budget reservation before create, and `bind_retry_thread` requires the authenticated create receipt. Resolve an actual approval or required response once only when its exact thread, idempotency key, fingerprint, account, destination, cap, OAuth scope, and existing user authorization match. A web-only approval may use an attended Hyperagent thread; never switch an agent to `Auto`, widen scopes, write memory, purchase credit, enable auto-recharge, or bypass a provider gate merely to unblock. Completed, failed, declined, and cancelled outcomes release exposure only through an authenticated exact-cost or zero-cost terminal receipt; unknown cost remains reserved. Remote completion is not landed; terminal success requires exact PR merge and required-runtime evidence.

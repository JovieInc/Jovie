...
Hyperagent remains one provider behind that router, never a parallel queue. Before `create_thread`, run `scripts/symphony/hyperagent/lifecycle.py preflight` against a fresh sanitized envelope and stop unless it returns `PROCEED`. A returned `threadId` is acceptance only: bind the issue, lease, idempotency key, attempt, request, destination, and required runtime in `LifecycleJournal`; observe that same thread with bounded backoff through `observe`; record PR, merge, and required-runtime evidence through `observe_delivery`; then reconcile the existing remote thread and PR before any retry. The pure plan never authorizes direct execution: reserve a mutation atomically with `reserve_action_once`, perform it only when that returns `execute: true`, and record its provider-result digest with `record_action_result`. Never infer an approval from prose. Keep actual approval, ordinary required input, memory approve/reject, sandbox-domain approval, stale status, transport loss, provider failure, remote completion, open PR, merge, and required-runtime proof distinct. An accepted or completed remote thread is not landed. Recover an existing PR through the native merge controller instead of redispatching its issue. `reserve_retry_once` may admit attempt 2 only when one revision-current receipt proves the remote job absent or idempotently replayable and no PR exists; bind the returned thread once with `bind_retry_thread`. Resolve an actual approval or required response once only when its exact thread, idempotency key, fingerprint, account, destination, cap, OAuth scope, and existing user authorization match. A web-only approval may use an attended Hyperagent thread; never switch an agent to `Auto`, widen scopes, write memory, purchase credit, enable auto-recharge, or bypass a provider gate merely to unblock. Terminal success requires the PR merge SHA to equal the exact required-runtime SHA with a durable receipt. A closed unmerged PR is terminal only with explicit failure owner and evidence; otherwise report `UNKNOWN`.

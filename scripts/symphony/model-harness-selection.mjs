...
export const MODEL_HARNESS_SELECTION_SCHEMA =
  'symphony-model-harness-selection/v1';
...
export function selectionPolicy(registry = readInvariantRegistry()) {
  const invariant = registry.invariants.find(
    item => item.policy?.key === POLICY_KEY
  );
  if (!invariant?.policy?.value)
    throw new Error('model-harness-policy-missing');
...
export function createExecutionAdapter(candidate) {
  ...
  if (typeof adapter.execute !== 'function')
    throw new Error('adapter-execute-required');
  if (adapter.capabilities.length === 0)
    throw new Error('adapter-capabilities-required');
...
export async function runShadowComparison({
  ...
  if (!incumbent || !challenger || incumbent === challenger)
    throw new Error('distinct-adapters-required');
  ...
  const [baselineResult, challengerResult] = await Promise.all([
    incumbent.execute(context),
    challenger.execute(context),
  ]);
  return {
    cohortId,
    workload,
...
function protectedRegression(baseline, challenger, policy) {
  return policy.protectedMetrics.some(
    metric =>
      challenger.metrics[metric] <
      baseline.metrics[metric] - policy.maxProtectedRegression
  );
}
...
export function decidePromotion({
  ...
  const reasons = [];
  const required = [
    ...policy.protectedMetrics,
    ...policy.primaryMetrics,
    ...policy.secondaryMetrics,
  ];
  if (!baseline || !challenger || !baseline.adapter || !challenger.adapter)
    reasons.push('missing-candidate-provenance');
  if (!baseline?.cohortId || baseline.cohortId !== challenger?.cohortId)
    reasons.push('cohort-mismatch');
  if (
    !Number.isInteger(challenger?.sampleSize) ||
    challenger.sampleSize < policy.minimumSampleSize
  )
    reasons.push('insufficient-samples');
  if (
    challenger?.confidence == null ||
    challenger.confidence < policy.minimumConfidence
  )
    reasons.push('insufficient-confidence');
  if (!challenger?.provenance?.version || !challenger.provenance?.source)
    reasons.push('missing-provenance');
  const observedAt = Date.parse(challenger?.observedAt || '');
  if (
    !Number.isFinite(observedAt) ||
    now - observedAt > policy.evidenceMaxAgeMs
  )
    reasons.push('stale-evidence');
  for (const metric of required)
    if (
      !validMetric(baseline?.metrics?.[metric]) ||
      !validMetric(challenger?.metrics?.[metric])
    )
      reasons.push(`invalid-metric:${metric}`);
  if (protectedRegression(baseline, challenger, policy))
    reasons.push('protected-metric-regression');
  const primaryDelta = Math.max(
    ...policy.primaryMetrics.map(
      metric => challenger?.metrics?.[metric] - baseline?.metrics?.[metric]
    ),
    -Infinity
  );
  const secondaryDelta = Math.max(
    ...policy.secondaryMetrics.map(
      metric => baseline?.metrics?.[metric] - challenger?.metrics?.[metric]
    ),
    -Infinity
  );
  if (
    primaryDelta < policy.materialPrimaryImprovement &&
    secondaryDelta < policy.materialSecondaryImprovement
  )
    reasons.push('non-material-delta');
  if (
    challenger?.lastPromotionAt &&
    now - Date.parse(challenger.lastPromotionAt) < policy.cooldownMs
  )
    reasons.push('hysteresis-cooldown');
  return {
...
export function buildComparisonReceipt({
  workload,
  cohortId,
  baseline,
  challenger,
  ...
  if (!decision || !baseline || !challenger)
    throw new Error('comparison-evidence-required');
  return {
    ...
    challenger: {
      ...
      sampleSize: challenger.sampleSize,
      confidence: challenger.confidence,
      provenance: challenger.provenance,
    },
    decision,
...
export function promoteFromReceipt({
  ...
  if (
    receipt?.schema !== COMPARISON_RECEIPT_SCHEMA ||
    receipt.decision?.decision !== 'promote'
  )
    throw new Error('promotion-receipt-required');
  if (
    !receipt.promotionScope?.workload ||
    receipt.promotionScope.workload !== receipt.workload
  )
    throw new Error('promotion-scope-invalid');
  const promotion = {
...
export function rollbackCertified({ selected, certified }) {
  if (!certified?.adapter || certified.certified !== true)
    throw new Error('certified-rollback-target-required');
  return {
    route: certified.adapter,
    reason: 'certified-floor-regression',

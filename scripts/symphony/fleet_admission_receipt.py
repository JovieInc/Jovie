...
def _project_controller_repair(value: object, promotion_mode: str) -> dict[str, Any]:
    ...
    if (
        projected["deploymentsAllowed"] is not False
        or projected["runtimeActivationAllowed"] is not False
    ):
        raise AdmissionProjectionError(
            "controller repair cannot authorize deployment or runtime"
        )
    if allowed:
        if promotion_mode != "controller-repair-only":
            raise AdmissionProjectionError(
                "controller repair authority contradicts promotionMode"
            )
        if projected["condition"] != "controller-failure":
            raise AdmissionProjectionError(
                "allowed controller repair condition is invalid"
            )
        ...
        if projected["maxConcurrent"] != 1:
            raise AdmissionProjectionError(
                "allowed controller repair must have maxConcurrent 1"
            )
        _hex_sha(projected["mainSha"], "controllerRepairAdmission.mainSha")
        deployed = projected["deployedSha"]
        if not isinstance(deployed, str) or len(deployed) < 7:
            raise AdmissionProjectionError(
                "allowed controller repair deployedSha is invalid"
            )
    elif promotion_mode == "controller-repair-only":
        raise AdmissionProjectionError(
            "controller-repair-only requires allowed repair authority"
        )
    elif any(
        projected[field] is not None
        for field in ("condition", "mainSha", "deployedSha", "scope")
    ) or projected["maxConcurrent"] != 0:
        raise AdmissionProjectionError(
            "denied controller repair must not carry authority"
        )
    return projected

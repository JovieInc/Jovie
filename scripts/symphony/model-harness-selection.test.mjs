...
const policy = {
  version: 1,
  protectedMetrics: ['correctness', 'firstPassGreen', 'safety'],
  primaryMetrics: ['correctness', 'firstPassGreen', 'safety'],
  secondaryMetrics: ['costEfficiency', 'latency'],
  minimumSampleSize: 3,
  minimumConfidence: 0.95,
  materialPrimaryImprovement: 0.05,
  materialSecondaryImprovement: 0.1,
  maxProtectedRegression: 0,
  evidenceMaxAgeMs: 60_000,
  cooldownMs: 60_000,
};
const adapter = (id, harness) =>
  createExecutionAdapter({
    id,
    model: 'k3',
...
const evidence = (a, metrics) => ({
  adapter: a,
  metrics,
  cohortId: 'representative-v1',
  sampleSize: 3,
  confidence: 0.99,
  provenance: { version: 'v1', source: 'replay' },
  observedAt: new Date().toISOString(),
});
const baseline = evidence(adapter('kimi-k3', 'kimi-cli'), {
  correctness: 0.8,
  firstPassGreen: 0.8,
  safety: 0.99,
  costEfficiency: 0.5,
...
describe('model × CLI/harness selection', () => {
  it('executes and compares K3 through Kimi and Codex adapters on one cohort', async () => {
    const challenger = evidence(adapter('codex-k3', 'codex-cli'), {
      correctness: 0.86,
      firstPassGreen: 0.86,
      safety: 0.99,
      costEfficiency: 0.5,
  ...
  it('rejects novelty-only, non-material, low-sample, stale, and protected-metric wins', () => {
    const challenger = evidence(adapter('new-route', 'new-cli'), {
      correctness: 0.8,
      firstPassGreen: 0.8,
      safety: 0.99,
      costEfficiency: 0.45,
    ...
    assert.equal(
      decidePromotion({ baseline, challenger, policy }).decision,
      'retain-incumbent'
    );
    assert.match(
      decidePromotion({
        baseline,
        challenger: { ...challenger, sampleSize: 1 },
        policy,
      }).reasons.join(','),
  ...
  it('promotes only from a comparison receipt and scopes the winner to one workload', () => {
    ...
    const decision = decidePromotion({ baseline, challenger, policy });
    const comparison = buildComparisonReceipt({
      workload: 'code',
      cohortId: baseline.cohortId,
      baseline,
      challenger,
      decision,
      trigger: 'explicit-registry-update',
    });
    const promotion = promoteFromReceipt({
      receipt: comparison,
      incumbent: baseline.adapter,
      policy,
    });
  ...
  it('fails closed for shadow mutation, lease theft, flapping, missing receipts, and uncertified rollback', () => {
    const shadow = createExecutionAdapter({
      ...
      execute: async context => {
        ...
        assert.equal(context.mutateProduction, false);
        assert.equal(context.activeLease, false);
        return {};
      },
    });
    assert.ok(shadow);
    ...
    assert.throws(
      () => promoteFromReceipt({ receipt: null, policy }),
      /promotion-receipt-required/
    );
    assert.throws(
      () =>
        rollbackCertified({
          selected: baseline.adapter,
          certified: { adapter: baseline.adapter, certified: false },
        }),
      /certified-rollback-target-required/
    );

...
export function buildAgentPrompt(input: BuildPromptInput): string {
  ...
  let result = [
    ...
    '- Writer-owned promotion must emit `jovie-writer-pr-proof/v1`, pair ready-for-review with native `node scripts/native-merge-intent.mjs --repo JovieInc/Jovie --pr <PR_NUMBER> --head` intent for the exact head, and emit `jovie-writer-pr-promotion-blocker/v1` on a blocked or uncertain request. Reconcile ambiguity without retry, dequeue, or draft compensation; only a definite rejected request may restore the same unchanged head to draft.',

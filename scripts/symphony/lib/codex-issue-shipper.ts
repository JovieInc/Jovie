...
export function buildAgentPrompt(input: BuildPromptInput): string {
  ...
  let result = [
    ...
    '- Writer-owned promotion must emit `jovie-writer-pr-proof/v1`, pair ready-for-review with native `node scripts/native-merge-intent.mjs --repo JovieInc/Jovie --pr <PR_NUMBER> --head` intent (`gh pr merge --auto --match-head-commit`) for the exact head, and emit `jovie-writer-pr-promotion-blocker/v1` on a blocked or uncertain request. Reconcile ambiguity without retry, dequeue, or draft compensation; only a definite rejected request may restore the same unchanged head to draft.',
  ...
  if (isUiUxDesignIssue(input.issue)) {
    result = result.concat([
      ...
      `- Safe UI-only fast-track lane: if and only if the final diff is limited to visual UI paths allowed by \`.github/MERGE_QUEUE.md\`, add PR labels \`ui\`, \`${UI_FAST_TRACK_LABEL}\`, and \`fast\` so the native queue controller can prioritize eligible UI work after evidence. Do not add the retired \`merge-queue\` label.`,
...
export const SHIPPER_CRITICAL_PATHS = [
  ...
  'scripts/symphony/lib/ship-ledger.ts',

...
export function buildAgentPrompt(input: BuildPromptInput): string {
  ...
  if (isUiUxDesignIssue(input.issue)) {
    result = result.concat([
      ...
      `- Safe UI-only fast-track lane: if and only if the final diff is limited to visual UI paths allowed by \`.github/MERGE_QUEUE.md\`, add PR labels \`ui\` and \`fast\` so the native queue controller can prioritize eligible UI work after evidence. Do not add the retired \`merge-queue\` or \`fast-track-ui\` labels.`,

...
describe('production-controller recovery runbook', () => {
  ...
  it('references only repo-relative paths that exist', () => {
    ...
    const matches = [
      ...runbook.matchAll(
        /(?:^|[\s`'"(])(?:scripts\/symphony\/|\.github\/|scripts\/lib\/|docs\/|apps\/web\/)[\w./-]+(?:\.\w+)?/gm
      ),
    ];
  ...
  it('is linked from the launchd README', () => {
    ...
    expect(readme).toContain('production-controller-recovery.md');
  });
});

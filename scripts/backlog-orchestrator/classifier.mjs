...
export class IssueClassification {
  ...
  #computeFingerprint(issue) {
    const canonical = [
      ...
      issue.state?.name || '',
      issue.labels?.nodes?.map(l => l.name).sort().join(',') || '',
      issue.updatedAt,
    ].join('|');
...
function classifyMrr(issue) {
  for (const label of issue.labels?.nodes || []) {
    ...
    if (mrr) return { category: mrr, confidence: 'medium', evidence: `label: ${label.name}` };
  }
  for (const [pattern, mrr] of TITLE_MRR_PATTERNS) {
    if (pattern.test(issue.title)) {
      return { category: mrr, confidence: 'high', evidence: `title match: ${pattern.source}` };
    }
  }
  return { category: 'unknown', confidence: 'low', evidence: 'no clear MRR signal' };
}

function classifyEffort(issue) {
  if (issue.estimate != null) {
    if (issue.estimate <= 1) return { effort: 'trivial', evidence: `estimate=${issue.estimate}` };
    if (issue.estimate <= 3) return { effort: 'small', evidence: `estimate=${issue.estimate}` };
    if (issue.estimate <= 8) return { effort: 'medium', evidence: `estimate=${issue.estimate}` };
  ...
  if (descLen < 100) return { effort: 'trivial', evidence: 'short description' };
  if (descLen < 500) return { effort: 'small', evidence: 'moderate description' };
  if (descLen < 2000) return { effort: 'medium', evidence: 'detailed description' };
...
function findExactDuplicates(issue, allIssues) {
  ...
  for (const rel of issue.relations?.nodes || []) {
    if (rel.type === 'duplicate' || rel.type === 'duplicate_of') {
      dupes.push({ identifier: rel.relatedIssue.identifier, relation: 'duplicate' });
    }
  }
  return dupes;
}
...
function findSuperseded(issue, allIssues) {
  ...
  const supersededBy = allIssues.filter(other => {
    ...
    if (other.state?.name === 'Canceled' || other.state?.name === 'Done') return false;
    ...
    return otherDate > issueDate && (other.priority || 0) > (issue.priority || 0);
  });
  return supersededBy.map(s => ({ identifier: s.identifier, relation: 'superseded_by' }));
}
...
function findObsolete(issue, allIssues) {
  const keyTerms = extractKeyTerms(issue.title + ' ' + (issue.description || ''));
  ...
  return matches.map(m => ({ identifier: m.identifier, relation: 'superseded_by' }));
}
...
function extractKeyTerms(text) {
  return text
    ...
    .replace(/[^a-z0-9\s-]/g, '')
    .split(/\s+/)
    .filter(w => w.length > 4 && !['this', 'that', 'with', 'from', 'have', 'been', 'were'].includes(w));
}
...
export function classifyDeterministic(issue, allIssues) {
  ...
  if (dupes.length > 0) {
    ...
    c.evidence.push(`exact duplicate of: ${dupes.map(d => d.identifier).join(', ')}`);
  }
  ...
  if (superseded.length > 0) {
    ...
    c.evidence.push(`superseded by: ${superseded.map(s => s.identifier).join(', ')}`);
  }
  ...
  if (obsolete.length > 0) {
    ...
    c.evidence.push(`obsolete — already satisfied by: ${obsolete.map(o => o.identifier).join(', ')}`);
  }

  // Related by URL
  const urlRelated = findRelatedByUrl(issue, allIssues);
  if (urlRelated.length > 0) {
    c.relatedIssues.push(...urlRelated.map(r => ({ ...r, relation: 'related' })));
    c.evidence.push(`shares URLs with: ${urlRelated.map(r => r.identifier).join(', ')}`);
  }
...
function computeValueScore(c) {
  ...
  const confidenceBonus = c.mrrConfidence === 'high' ? 15 : c.mrrConfidence === 'medium' ? 5 : 0;
  return Math.max(0, Math.min(100, base - effort + confidenceBonus));
}

function computeRisk(c) {
  if (c.effort === 'large' && c.mrrCategory === 'revenue-protection') return 'high';
...
export function parseStoredClassification(issue) {
  const machineComments = (issue.comments?.nodes || []).filter(
    c => c.body.startsWith('<!-- backlog-orchestrator:v1 -->')
  );
  if (machineComments.length === 0) return null;
  const latest = machineComments.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0];
  try {
    const jsonPart = latest.body.split('<!--/backlog-orchestrator-->')[0]?.split('-->')[1]?.trim();

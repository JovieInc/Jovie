...
function createApi() {
  ...
  const issueKey = signal => identifiedKey(signal) || (repositoryName(signal) && text(signal.deliveryKey) ? `${repositoryName(signal)}:${text(signal.deliveryKey)}` : null) || anonymousIdentity(signal);
  ...
  function markNotProven(record, reason, now) {
    ...
    return { ...record, repository: classified.repository, issueKey: classified.issueKey, stallClass: 'not-proven', mode: 'collect-evidence', owner: classified.owner, writer: classified.writer, action: classified.action, proven: false, dispatchState: 'classified', outcome: 'open', terminal: false, reason, observedAt: iso(now), nextProofAt: nextProofAt(iso(now), Math.max(1, record.attempt || 0)) };
  }
  ...
  function escalate(record, reason, now = new Date().toISOString()) {
    ...
    return { ...record, mode: 'authority-blocker', outcome: 'escalated', terminal: true, dispatchState: 'escalated', owner: 'human', writer: 'human', action: 'visible-actionable-escalation', reason: exactReason, authorityBudget: 0, observedAt: iso(now), escalation: { reason: exactReason, repository: record.repository, stallClass: record.stallClass, issue: record.issue, pr: record.pr, headSha: record.headSha, attempts: record.attempt } };
  }
  ...
  function projectSummerQueue(records, { now = new Date().toISOString() } = {}) {
    ...
    for (const record of source.filter(record => record.outcome !== 'healthy' && repoName(record.repository))) {
    ...
    const terminalTombstones = source.filter(record => record.outcome === 'healthy' && repoName(record.repository)).map(record => ({ ...Object.fromEntries(QUEUE_KEYS.map(key => [key, record[key]])), issue: record.issue, pr: record.pr, outcome: record.outcome, terminal: record.terminal, observedAt: record.observedAt, reason: record.reason }));

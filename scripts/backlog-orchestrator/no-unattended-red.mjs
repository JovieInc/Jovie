...
function createApi() {
  ...
  function projectSummerQueue(records, { now = new Date().toISOString() } = {}) {
    ...
    const terminalTombstones = [
      ...source.filter(record => record.stallClass !== 'draft-stack-policy' && record.outcome === 'healthy' && repoName(record.repository)),
      ...[...draftStacks.values()].filter(record => record.outcome === 'healthy' && repoName(record.repository)),

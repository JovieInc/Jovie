...
export async function readShippingLeadIssue(task, { client }) {
  ...
  if (
    ...
    throw new Error('shipping-lead-canonical-lease-incomplete');
  const allLeases = issue.comments.nodes
  ...
  if (
    allLeases.some(
      candidate =>
        ...
        (Date.parse(candidate.at) >= Date.parse(lease.at) &&
          shippingDigest(candidate) !== leaseDigest)
    )
  )
    throw new Error('shipping-lead-canonical-lease-superseded');
  return {
    issueId: issue.id,

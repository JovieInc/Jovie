...
import { buildDeliveryReceipt, transitionDeliveryReceipt } from '../delivery-state-machine.mjs';
import { projectShippingChain, projectShippingPortfolio, UNKNOWN } from '../shipping-observability.mjs';
...
function completeChain() {
  ...
  const leased = transitionDeliveryReceipt(
    received,
    {
      stage: 'leased',
      evidence: { shipping: shipping('executor', 'agent-17') },
    },
    { now: '2026-09-08T20:01:00.000Z' }
  );
  const ci = transitionDeliveryReceipt(
    leased,
    {
      stage: 'ci-pending',
      evidence: { shipping: shipping('reviewer', 'review-agent-3') },
    },
    { now: '2026-09-08T20:03:00.000Z' }
  );
  const merged = transitionDeliveryReceipt(
    ci,
    {
      stage: 'merged',
      evidence: { shipping: shipping('mergeActor', 'github-merge-queue') },
    },
    { now: '2026-09-08T20:05:00.000Z' }
  );
  const production = transitionDeliveryReceipt(
    merged,
    {
      stage: 'production-proven',
      deployedSha: HEAD,
      evidence: { shipping: shipping('deploymentActor', 'vercel-production') },
    },
    { now: NOW }
  );
...
describe('shipping observability projection', () => {
  ...
  it('qualifies missing, mismatched, stale, and disconnected evidence as UNKNOWN', () => {
    ...
    assert.equal(projectShippingChain(missing, { now: NOW }).qualification.status, UNKNOWN);
    ...
    assert.equal(projectShippingChain(mismatch, { now: NOW }).qualification.status, UNKNOWN);
    assert.match(
      projectShippingChain(completeChain(), { now: '2026-09-08T21:00:01.000Z' }).qualification.reason,
      /stale-observation/
    );
    const disconnected = completeChain();
    disconnected[2] = { ...disconnected[2], previousReceiptKey: 'missing' };
    assert.equal(projectShippingChain(disconnected, { now: NOW }).qualification.status, UNKNOWN);
  });
  ...
  it('reports retry causes and the oldest qualified blocked chain', () => {
    ...
    const blocked = transitionDeliveryReceipt(
      received,
      {
        stage: 'repair-pending',
        failure: 'ci-failed',
        evidence: {
          shipping: shipping('coordinator', 'summer', {
            execution: undefined,

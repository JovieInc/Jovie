...
const REQUIRED_ROLES = Object.freeze(['coordinator', 'executor', 'reviewer', 'mergeActor', 'deploymentActor']);
const ROLE_REQUIRED_STAGES = Object.freeze({
  ...
  reviewer: new Set(['ci-pending', 'queue-pending', 'queued', 'merged', 'deployment-pending', 'production-proven']),
...
function shippingEvidence(receipt) {
  const value = receipt?.transition?.evidence?.shipping || receipt?.event?.evidence?.shipping;
...
function executionEvidence(value) {
  ...
  const normalized = Object.fromEntries(fields.map((field) => [field, text(value[field], 160)]));
  return fields.every((field) => normalized[field]) ? { ...normalized, state: value.state } : null;
}

function valueEvidence(value) {
  if (!value || !['founder-request', 'summer-priority'].includes(value.authority)) return null;
...
function referenceEvidence(receipt, value) {
  ...
  if (!issue || !pr || !/^[0-9a-f]{40}$/.test(headSha || '') || !runId) return null;
  if (issue !== receipt.event.issue || pr !== receipt.event.pr || headSha !== receipt.event.headSha) return null;
  return {
    issue,
    pr,
    headSha,
...
function orderedChain(receipts) {
  if (!Array.isArray(receipts) || receipts.length === 0) return { error: 'receipt-chain-empty' };
  const byKey = new Map(receipts.map((receipt) => [receipt?.receiptKey, receipt]));
  if (byKey.size !== receipts.length || byKey.has(undefined)) return { error: 'receipt-key-missing-or-duplicate' };
  const children = new Map();
  for (const receipt of receipts) {
    if (receipt?.schema !== DELIVERY_RECEIPT_SCHEMA) return { error: 'receipt-schema-invalid' };
    if (receipt.previousReceiptKey) {
      if (!byKey.has(receipt.previousReceiptKey)) return { error: 'receipt-chain-incomplete' };
      if (children.has(receipt.previousReceiptKey)) return { error: 'receipt-chain-forked' };
  ...
  if (ordered.length !== receipts.length) return { error: 'receipt-chain-disconnected' };
  for (let index = 0; index < ordered.length; index += 1) {
    const observedAt = time(ordered[index].observedAt);
    if (observedAt === null || (index && observedAt < time(ordered[index - 1].observedAt))) {
...
export function projectShippingChain(receipts, { now = new Date().toISOString(), staleAfterMs = 30 * 60 * 1000 } = {}) {
  const chain = orderedChain(receipts);
  if (chain.error) {
    return {
      schema: SHIPPING_OBSERVABILITY_SCHEMA,
      qualification: unknown(chain.error, 'Gem', 'repair-receipt-chain-instrumentation'),
    };
  }
  ...
  const actors = Object.fromEntries(
    REQUIRED_ROLES.map((role) => [
      role,
      unknown('actor-evidence-missing', role === 'coordinator' ? 'Summer' : 'Gem', 'record-actual-stage-actor'),
    ])
  );
  let execution = unknown('execution-evidence-missing', 'Symphony', 'record-provider-model-harness-task-attempt');
  let value = unknown('value-justification-missing', 'Summer', 'record-founder-request-or-summer-priority');
  let refs = unknown('source-pr-run-evidence-missing', 'Gem', 'record-exact-source-pr-run-binding');
  ...
  for (const receipt of ordered) {
    ...
    if (executionCandidate) execution = measured(executionCandidate, evidence.execution.evidence || receipt.receiptKey);
    ...
    const retryCount = Number.isInteger(evidence?.retry?.count) && evidence.retry.count >= 0 ? evidence.retry.count : 0;
    retries += retryCount;
    const cause = text(evidence?.retry?.cause, 120);
    if (retryCount && cause) failureCauses[cause] = (failureCauses[cause] || 0) + retryCount;
  }
  ...
  const defects = [
    ...Object.entries(actors)
      .filter(
        ([role, field]) =>
          field.status === UNKNOWN && [...ROLE_REQUIRED_STAGES[role]].some((stage) => reachedStages.has(stage))
      )
      .map(([role]) => `missing-${role}`),
    ...(execution.status === UNKNOWN && [...reachedStages].some((stage) => ROLE_REQUIRED_STAGES.executor.has(stage))
      ? ['missing-execution']
      : []),
  ...
  return {
    ...
    stage: last.stage,
    terminal,
    executionState: execution.status === 'measured' ? execution.value.state : UNKNOWN,
    actors,
    execution,
    value,
    refs,
    stageTimings,
    leadTimeMs: measured(time(last.observedAt) - time(first.observedAt), `${first.receiptKey}:${last.receiptKey}`),
    retries: measured(
      retries,
...
export function projectShippingPortfolio(
  chains,
  { now = new Date().toISOString(), windowMs = 7 * 24 * 60 * 60 * 1000, staleAfterMs } = {}
) {
  ...
  const qualified = items.filter((item) => item.qualification?.status === 'measured');
  const completed = qualified.filter((item) => item.terminal && time(item.observedAt) >= cutoff);
  ...
  return {
    ...
    qualification: unqualified.length
      ? unknown(`${unqualified.length}-unqualified-chain(s)`, 'Gem', 'repair-chain-evidence-before-portfolio-claim')
      : measured(
          'qualified',

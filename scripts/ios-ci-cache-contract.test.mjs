...
test('iOS CI restores only immutable Swift package state before dependency resolution', () => {
  const cacheStart = workflow.indexOf('      - name: Restore Swift package cache');
  const resolveStart = workflow.indexOf('      - name: Resolve Swift package dependencies');

  assert.ok(cacheStart >= 0, 'SwiftPM cache step is required');
  assert.ok(resolveStart > cacheStart, 'SwiftPM cache must restore before resolution');

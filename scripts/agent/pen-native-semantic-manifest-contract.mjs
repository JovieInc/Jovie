...
export function validatePenNativeSemanticManifestReceipt(
  ...
  if (!record(receipt)) {
    return {
      valid: false,
      errors: [{ code: 'receipt_not_object', message: 'Receipt must be JSON.' }],
    };
  }
  ...
  if (!penPath(sourcePath))
    add(errors, 'source_path_invalid', 'Source path must be an absolute .pen path.');
  if (expectedPath && sourcePath !== expectedPath)
    add(errors, 'source_path_mismatch', 'Source path differs from the locked path.');

  for (const field of ['source_byte_identity', 'canonical_manifest_sha256']) {
    if (!digest(receipt[field]))
      add(errors, `${field}_invalid`, `${field} must be a lowercase SHA-256 digest.`);
  }
  for (const field of ['runtime_identity', 'build_identity']) {
    if (!text(receipt[field])) add(errors, `${field}_missing`, `${field} is required.`);
  }
  ...
  if (!record(deltas)) {
    ...
    for (const field of [
      ...
      if (deltas[field] !== 0)
        add(errors, `event_delta_${field}_invalid`, `${field} delta must be zero.`);
    }
  }
  ...
  if (!Array.isArray(nodes) || !Array.isArray(roots)) {
    add(errors, 'manifest_graph_invalid', 'Manifest must contain nodes and root_ids arrays.');
  ...
  for (const node of nodes) {
    ...
    if (!record(node) || !id || byId.has(id)) {
      add(errors, 'node_identity_invalid', 'Every node must have a unique non-empty ID.');
      continue;
    }
    byId.set(id, node);
    ...
    if (typeof node.reusable !== 'boolean')
      add(errors, 'node_reusable_invalid', 'Every node requires a reusable boolean.');
    ...
    if (!digest(node.properties_sha256))
      add(errors, 'node_properties_digest_invalid', 'Every node requires a properties SHA-256 digest.');
    if (!Array.isArray(node.child_ids) || new Set(node.child_ids).size !== node.child_ids.length)
      add(errors, 'node_children_invalid', 'Every node requires unique ordered child_ids.');
  }
  if (new Set(roots).size !== roots.length || roots.some(root => !byId.has(root)))
    add(errors, 'root_ids_invalid', 'Root IDs must be unique and refer to nodes.');
  ...
  const visit = id => {
    ...
    for (const childId of node.child_ids ?? []) {
      if (!byId.has(childId)) add(errors, 'child_id_missing', 'Every child ID must refer to a node.');
      else visit(childId);
    }
    visiting.delete(id);
  };
  ...
  if (reachable.size !== nodes.length)
    add(errors, 'manifest_truncated', 'Every node must be reachable from a root.');
  ...
  if (manifest.total_node_count !== nodes.length)
    add(errors, 'total_node_count_invalid', 'total_node_count must equal node count.');
  const reusableCount = nodes.filter(node => node.reusable === true).length;
  if (manifest.reusable_count !== reusableCount)
    add(errors, 'reusable_count_invalid', 'reusable_count must match reusable nodes.');
...
export function diffPenSemanticManifests(left, right) {
  ...
  const removed = [...leftNodes.keys()].filter(id => !rightNodes.has(id)).sort();
  const changed = [...leftNodes.keys()]
    .filter(id => rightNodes.has(id))
    .filter(id => leftNodes.get(id).properties_sha256 !== rightNodes.get(id).properties_sha256)
    .sort();

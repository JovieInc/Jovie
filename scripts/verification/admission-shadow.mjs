...
export function evaluateProviderQualification(packet, { now }) {
  ...
  if (blockers.length > 0) return { qualified: false, blockers, digest };
  if (/^(latest|default|auto)$/i.test(packet.model)) {
    blockers.push('model-must-be-pinned');
  }
  if (!packet.principal.revocable || !packet.principal.credentialRef) {
  ...
  if (
    ...
    packet.receipt?.requestDigest !== true ||
    packet.receipt?.responseDigest !== true ||
    packet.receipt?.toolCallDigest !== true
  ) {
  ...
  if (!Number.isFinite(packet.budget?.maxCostUsd)) {
    blockers.push('provider-budget-required');
  }
  if (
    !Number.isFinite(Date.parse(packet.expiresAt)) ||
    Date.parse(packet.expiresAt) <= Date.parse(now)
  ) {
...
export function deriveShadowCertificate({
  ...
  const validEntries = [];
  for (const entry of activeEvidence(entries)) {
    const errors = validateAuditEvidenceShape(entry.evidence);
    if (errors.length > 0) {
      blockers.push('malformed_evidence');
      continue;
    }
    ...
    if (
      ...
      digestObject(evidence.subject.artifactDigests ?? []) ===
        digestObject(currentSubject.artifactDigests ?? [])
    ) {
      validEntries.push(entry);
    }
  }
  const latest = validEntries.at(-1)?.evidence;
  if (!latest) blockers.push('missing_current_evidence');
  else {
    if (NON_PASS_OUTCOMES.includes(latest.outcome))
      blockers.push(latest.outcome);
    try {

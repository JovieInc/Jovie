 ...
 *  5. For each `issue` span → file a GitHub issue (Linear mirror when enabled).
...
async function fileIssueFromSpan(args: {
  ...
  // Tracker is down but the intent landed in the retry queue. Treat as
...
async function ingestFile(filePath: string): Promise<void> {
  ...
  // Only mark the memo seen if every issue span landed in the tracker (a

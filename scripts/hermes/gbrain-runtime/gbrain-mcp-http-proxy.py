...
def validate_mcp_url() -> None:
    ...
    try:
        ...
        raise RuntimeError("GBRAIN_MCP_URL port must be valid") from exc
...
def read_token_file() -> str:
    ...
    try:
        ...
        token_bytes = b""
        while len(token_bytes) < metadata.st_size:
            chunk = os.read(descriptor, metadata.st_size - len(token_bytes))
            if not chunk:
                raise RuntimeError("token file changed while reading")
            token_bytes += chunk
        try:
...
def forward(payload: dict, token: str, total_deadline: float) -> None:
    global PROTOCOL_VERSION, SESSION_ID
                    ...
                    if "id" in response_payload:
                        ...
                        if payload.get("method") == "initialize":
                            ...
                            if isinstance(result_payload, dict):
                                protocol_version = result_payload.get(
                                    "protocolVersion"
                                )
...
def handle_line(raw_line: bytes, token: str, total_deadline: float) -> None:
    ...
    try:
        ...
        if payload.get("jsonrpc") != "2.0" or not isinstance(
            ...
            emit(error_response(request_id, "invalid JSON-RPC request"))
            return
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while True:
            ...
            if not raw_line:
                break
            if len(raw_line) > MAX_REQUEST_BYTES:  # pragma: no cover
            ...
            if not capacity.acquire(timeout=request_deadline):  # pragma: no cover
                os._exit(1)

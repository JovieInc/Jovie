...
SESSION_PHASES = ACTIVE_PHASES | {"confirmation", "succeeded", "failed", "expired"}
...
def parse_device_auth(chunk):
    if SECRETISH.search(chunk):
        chunk = SECRETISH.sub("[redacted]", chunk)
    uri = DEVICE_URI_RE.search(chunk)
    code = None
    for candidate in USER_CODE_RE.findall(chunk):
        if candidate.upper() not in {"HTTP-404", "UTF8-BOM"}:
            code = candidate
...
def _binding_review():
    raw_home = None
    try:
        for line in _expand("SYMPHONY_CODEX_ACCOUNT_ENV", "~/.config/symphony/codex-account.env").read_text(
            encoding="utf-8"
        ).splitlines():
            stripped = line.strip()
...
def _active_session():
    newest = None
    try:
        entries = list((_root() / "control").glob("session-*.json"))
    except OSError:
        return None
    for path in entries:
...
def inspect_snapshot():
    ...
    for label in APPROVED_LABELS:
        ...
        accounts.append({
            ...
            "reconnectEligible": True, "authPresent": auth_present,
        })
    return {
...
def public_session(session):
    ...
    return {
        ...
        "expiresAt": session.get("expiresAtIso") or _iso(int(session.get("expiresAt") or _now() + SESSION_TTL_SECONDS)),
        "receipt": {
...
def start_reconnect(account):
    ...
    if account not in APPROVED_LABELS:
        snapshot["error"] = "account_not_approved"
    ...
    while time.time() < deadline:
        ...
        if latest.get("userCode") or latest.get("phase") in {"failed", "expired", "succeeded"}:
            break
...
def _supervise(account, session_id):
    ...
    try:
        ...
        collected = ""
        deadline = int(session.get("expiresAt") or (_now() + SESSION_TTL_SECONDS))
        assert proc.stdout is not None
        while True:
            if _now() >= deadline:
                proc.kill()
                _save(path, session, phase="expired", error="expired")
                return 0
            ...
            if line:
                ...
                if code and session.get("userCode") != code:
                    updated["userCode"] = code
                if uri and session.get("verificationUri") != uri:
                    updated["verificationUri"] = uri
                if updated:
                    _save(path, session, **updated)
...
def main(argv=None):
    ...
    if command == "reconnect":
        account = rest[rest.index("--account") + 1] if "--account" in rest and rest.index("--account") + 1 < len(rest) else ""
        return emit(start_reconnect(account))
    if command == "_supervise" and len(rest) >= 2:
        return _supervise(rest[0], rest[1])
    return 2

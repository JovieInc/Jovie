...
class SymphonyCodexAccountControlTests(unittest.TestCase):
    ...
    def test_inspect_lists_only_approved_labels_and_keeps_states_distinct(self):
        ...
        self.assertEqual(by_label["meetjovie"], "usage-exhausted")
        self.assertEqual(by_label["jovie"], "stale")
        self.assertEqual(by_label["timwhite-co"], "unknown")
        binding = payload["binding"]
        self.assertEqual(binding["boundLabel"], "personal")
        self.assertFalse(binding["recognized"])
        self.assertFalse(binding["selectable"])
        self.assertFalse(binding["canSwitch"])
        self.assertFalse(binding["canRestart"])
    ...
    def test_reconnect_is_locked_secret_free_and_emits_selected_account_receipt(self):
        ...
        while time.time() < deadline:
            inspected = json.loads(self.run_helper("inspect").stdout)
            current = inspected.get("session") or {}
    ...
    def test_unapproved_account_stays_unselectable(self):
        ...
        self.assertEqual(
            [row["label"] for row in payload["accounts"]],
            ["meetjovie", "jovie", "timwhite-co"],
        )
    ...
    def test_device_auth_parser_drops_secret_material(self):
        ...
        self.assertEqual(uri, "https://auth.openai.com/codex/device")
        self.assertEqual(
            module.classify_account_state(
                auth_present=True,
                cooldown_until=int(time.time()) + 10,
                readiness_expires=int(time.time()) + 10,
                now=int(time.time()),
            ),
            "usage-exhausted",
        )

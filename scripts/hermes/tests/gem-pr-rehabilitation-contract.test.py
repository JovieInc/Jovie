...
class DeploymentContractTests(unittest.TestCase):
    ...
    def test_installer_and_activation_require_timer_persistence(self):
        ...
        workflow = ACTIVATION.read_text(encoding="utf-8")
        self.assertIn('systemctl --user enable --now "${TIMER}"', installer)
        self.assertIn('systemctl --user is-enabled --quiet "${TIMER}"', installer)
        self.assertIn('"timerEnabled": True', installer)
        self.assertIn("timer_was_enabled", installer)
        self.assertIn('systemctl --user disable "${TIMER}"', installer)
        self.assertIn(".timerEnabled == true", workflow)
        self.assertIn("is-enabled --quiet gem-pr-drain.timer", workflow)
        self.assertIn("--property=UnitFileState --value", workflow)
        self.assertIn("= enabled", workflow)

    def test_installer_and_activation_require_timer_liveness(self):
        installer = INSTALLER.read_text(encoding="utf-8")
        workflow = ACTIVATION.read_text(encoding="utf-8")
        self.assertIn('systemctl --user is-active --quiet "${TIMER}"', installer)
        self.assertIn('"timerActive": True', installer)
        self.assertIn(".timerActive == true", workflow)
        self.assertIn("is-active --quiet gem-pr-drain.timer", workflow)

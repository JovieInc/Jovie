...
function validMetadata(metadata) {
  return metadata && typeof metadata === 'object'
    ...
    && typeof metadata.run_id === 'string' && metadata.run_id.length > 0
...
export function evaluateCandidate(worktree, options = {}) {
  ...
  const result = { eligible: false, ttlDays, alert: pressureAlert(policy, freeGb) };
  if (worktree.currentMain) return { ...result, reason: 'current_main' };
  if (worktree.namedUser || worktree.metadata?.named_user === true) return { ...result, reason: 'named_user' };
  ...
  if (!validMetadata(worktree.metadata)) return { ...result, reason: 'missing_metadata' };
  if (worktree.metadata.owner !== 'unclaimed' || worktree.metadata.claimed === true) {
    return { ...result, reason: 'claimed' };
  }
  const activity = Date.parse(worktree.metadata.last_activity_at ?? worktree.metadata.created_at);
  const ageDays = (now - activity) / 86400000;
  if (!Number.isFinite(ageDays) || ageDays < ttlDays) {
    return { ...result, reason: 'ttl_not_reached', ageDays: Math.max(0, ageDays) };
  }
...
function readMetadata(worktreePath, metadataFile) {
  try { return JSON.parse(fs.readFileSync(path.join(worktreePath, metadataFile), 'utf8')); }
  catch { return null; }
}
...
function activeProcessPaths() {
  try {
    const output = execFileSync('lsof', ['-Fn', '-a', '-d', 'cwd'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
...
export function inventory(root, options = {}) {
  const policy = { ...DEFAULT_POLICY, ...(options.policy ?? {}) };
  const repoRoot = path.resolve(root ?? runGit(process.cwd(), ['rev-parse', '--show-toplevel']));
  const freeGb = options.freeGb ?? diskFreeGb(repoRoot) ?? Infinity;
  const processPaths = options.processPaths ?? activeProcessPaths();
  const entries = parseWorktreePorcelain(runGit(repoRoot, ['worktree', 'list', '--porcelain']));
  const worktrees = entries.map((entry) => {
    const worktreePath = path.resolve(entry.path);
    let dirty = false;
    try { dirty = runGit(worktreePath, ['status', '--porcelain', '--untracked-files=normal']) !== ''; } catch { dirty = true; }
    const metadata = readMetadata(worktreePath, policy.metadataFile);
    const currentMain = worktreePath === repoRoot || entry.branch === 'refs/heads/main';
    const activeProcess = processPaths.some((p) => p === worktreePath || p.startsWith(`${worktreePath}${path.sep}`));
    const decision = evaluateCandidate({ ...entry, path: worktreePath, dirty, metadata, currentMain, activeProcess }, { ...options, policy, freeGb });
    return { ...entry, path: worktreePath, dirty, currentMain, activeProcess, namedUser: metadata?.named_user === true, metadata, decision };
  });
  return {
    schema_version: 1,
...
export function registerWorktree(worktreePath, { owner, runId, createdAt, namedUser = false }) {
  ...
  fs.writeFileSync(path.join(worktreePath, DEFAULT_POLICY.metadataFile), `${JSON.stringify(metadata, null, 2)}\n`, { mode: 0o600 });
  return metadata;
}
...
export function reap(root, options = {}) {
  ...
  if (options.apply === true) {
    for (const worktree of report.worktrees.filter((item) => item.decision.eligible)) {
      try {
        execFileSync('git', ['-C', report.root, 'worktree', 'remove', worktree.path], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
        removals.push({ path: worktree.path, removed: true });
      } catch (error) {
        removals.push({ path: worktree.path, removed: false, error: error.message });
      }
    }
    if (removals.some((item) => item.removed)) {
      execFileSync('git', ['-C', report.root, 'worktree', 'prune'], { encoding: 'utf8' });

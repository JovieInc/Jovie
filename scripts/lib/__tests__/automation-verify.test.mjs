...
const PREREQUISITE_TRAIN_MANIFEST = [
  ...
  'apps/web/components/features/onboarding/OnboardingShell.tsx',
  'apps/web/components/features/onboarding/useOnboardingClaim.ts',
  'apps/web/components/features/auth/GoogleOneTap.tsx',
  ...
  'apps/web/lib/onboarding/session.ts',
  'apps/web/lib/testing/e2e-prebuilt-claim.ts',
  ...
  'apps/web/tests/lib/legal/getLegalDocument.test.ts',
  ...
  'apps/web/tests/unit/api/onboarding/claim.test.ts',
  ...
  'apps/web/tests/unit/profile/profile-links-dedupe.test.tsx',
  'apps/web/tests/unit/seo/seo-ratchet.test.ts',
...
describe('automation-verify affected scope', () => {
  ...
  it('lets the prerequisite train extend the selector on focused contracts', () => {
    ...
    expect(plan.mode).toBe('selected');
    expect(plan.relatedFiles).toHaveLength(52);
    expect(plan.mandatoryTests).toEqual([
      'apps/web/lib/events/confirmation-status.test.ts',
      'apps/web/tests/unit/events/insert.test.ts',
      'apps/web/tests/unit/testing/seed-test-data-import-boundary.test.ts',

...
describe('rolling CI defect-class learning', () => {
  ...
  it('requires every learning field before a repaired class can close', () => {
    expect(
      ...
      expect.arrayContaining([
        ...
        'minimalRepair is required',
        'equivalent-surface sweep is required',
      ])
    );
  });
  ...
  it('routes environment failures to execution or runner repair', () => {
    ...
    expect(
      validateLearningReceipt({
        ...
        environmentRemediation: {
          ...
          runnerFix: 'restore macos-15-xlarge signing entitlements',
        },
      }).ok
    ).toBe(true);
    expect(
    ...
    expect(
      validateLearningReceipt({
        ...environment,
        environmentRemediation: { incidentClass: 'execution-path' },
      }).errors
    ).toContain('execution-path incidents require a classifier');
  });
  ...
  it('does not treat required CI as a learning-gate input', () => {
    expect(
      evaluateLearningPromotion({
        repairedFailures: [],
        receipts: [],
    ...
    ).toMatchObject({ complete: true, requiredReceipts: 0, blockers: [] });
  });

...
describe('repository-owned GBrain runtime assets', () => {
  ...
  it('emits an SSE event before the server closes the stream', async () => {
    ...
    const port = await listen((_request, response) => {
      ...
      response.writeHead(200, { 'content-type': 'text/event-stream' });
      response.write(
        'data: {"jsonrpc":"2.0","id":15,"result":{"ok":true}}\n\n'
      );
    });
    ...
    const result = await new Promise((resolveRun, rejectRun) => {
      ...
      const timeout = setTimeout(() => {
        child.kill('SIGKILL');
        rejectRun(
          new Error('proxy did not emit while SSE stream remained open')
        );
      }, 3000);
      child.stderr.on('data', chunk => {
        stderr += chunk;
      ...
      child.stdin.write('{"jsonrpc":"2.0","id":15,"method":"tools/list"}\n');
    });
  ...
  it('refuses to send a bearer token to any remote host', async () => {
    const result = await runProxy({

...
function runProcess(command, args, { env = {}, unset = [], stdin = '' } = {}) {
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.stdin.on('error', error => {
      if (!('code' in error) || error.code !== 'EPIPE') rejectRun(error);
    });
...
async function runProxy({
  ...
  tokenFile: suppliedTokenFile = undefined,
  tokenMode = 0o600,
  createToken = true,
  payload = undefined,
  rawInput = undefined,
...
describe('repository-owned GBrain runtime assets', () => {
  it('keeps the candidate service loopback-only and secret-free', async () => {
    ...
    expect(proxy).not.toContain('os.fdopen');
    expect(assets).not.toContain('/ready');

    const wrapperPort = wrapper.match(
      /GBRAIN_SERVE_PORT="\$\{[^}]+:-([0-9]+)\}"/
    );
  ...
  it('fails closed before launching when operator config is malformed', async () => {
    ...
    await writeFile(fakeBinary, `#!/usr/bin/env bash\ntouch "$MARKER_FILE"\n`, {
      mode: 0o755,
    });
  ...
  it('preserves server session IDs after an initialize SSE event', async () => {
    let followupSessionId;
    let followupProtocolVersion;
    const port = await listen(async (request, response) => {
      const chunks = [];
      for await (const chunk of request) chunks.push(chunk);
      const payload = JSON.parse(Buffer.concat(chunks).toString('utf8'));
      ...
      if (payload.method === 'initialize') {
        ...
        response.end(
          'data: {"jsonrpc":"2.0","id":15,"result":{"ok":true,"protocolVersion":"2025-06-18"}}\n\n'
        );
        return;
      }

      followupSessionId = request.headers['mcp-session-id'];
      followupProtocolVersion = request.headers['mcp-protocol-version'];
      response.writeHead(200, { 'content-type': 'application/json' });
      response.end('{"jsonrpc":"2.0","id":16,"result":{"ok":true}}');
    });
    const result = await runProxy({
      port,
      rawInput:
      ...
      extraEnv: { GBRAIN_MCP_MAX_WORKERS: '1' },
    });
    ...
    expect(result.code).toBe(0);
    expect(result.stderr).toBe('');
    expect(followupSessionId).toBe('test-session');
    expect(followupProtocolVersion).toBe('2025-06-18');
    expect(responses).toEqual([
      {
        jsonrpc: '2.0',
        id: 15,
        result: { ok: true, protocolVersion: '2025-06-18' },
      },
      { jsonrpc: '2.0', id: 16, result: { ok: true } },
    ]);
  }, 5000);

  it('keeps retry and post-response error handling deadline-aware', async () => {
    const proxy = await readFile(proxyPath, 'utf8');

    expect(proxy).toContain(
      'time.sleep(max(0.0, min(delay, total_deadline - time.monotonic())))'
    );
    expect(proxy).toContain(
  ...
  it.each([
    ['empty', 200, ''],
    ['mismatched-id', 200, '{"jsonrpc":"2.0","id":999,"result":{"ok":true}}'],
    ['scalar', 200, '42'],
    ...
    ['scalar-error', 200, '{"jsonrpc":"2.0","id":17,"error":"not-an-object"}'],
    ...
    [
      'boolean-response-id',
      200,
      '{"jsonrpc":"2.0","id":true,"result":{"ok":true}}',
    ],
    [
      'null-response-id',
      200,
  ...
  it.each([
    ['remote host', 'http://example.invalid/mcp', 'refusing non-loopback'],
    ['malformed port', 'http://127.0.0.1:bad/mcp', 'port must be valid'],
  ])('fails startup for %s', async (_name, url, message) => {
    const result = await runProxy({ url, rawInput: '' });
  ...
  it.each([
    ...
    {},
    [],
  ])('rejects invalid request id %j before forwarding', async id => {
    let requests = 0;
    const port = await listen((_request, response) => {
      requests += 1;
    ...
    const result = await runProxy({
      port,
      payload: { jsonrpc: '2.0', id, method: id === true ? 7 : 'tools/list' },
    });

    expect(requests).toBe(0);
    expect(JSON.parse(result.stdout)).toMatchObject({
      id: null,
      error: { code: -32000, message: 'invalid JSON-RPC request id' },
    });
  });
  ...
  it('terminates the transport for an oversized request so pending calls reject', async () => {
    const port = await listen((_request, response) => {
      response.writeHead(200, { 'content-type': 'text/event-stream' });
      const interval = setInterval(() => response.write(': active\n\n'), 100);
    ...
    const result = await runProxy({
      ...
      extraEnv: { GBRAIN_MCP_REQUEST_DEADLINE_SECONDS: '5' },
    });
  ...
  it.each([
    'application/json',
    'text/event-stream',
  ])('rejects an oversized %s daemon response', async contentType => {
    const port = await listen((_request, response) => {
  ...
  it.each([
    'text/event-stream',
    'application/json',
  ])('interrupts a silent %s read at the absolute request deadline', async contentType => {
    const port = await listen((_request, response) => {
      response.writeHead(200, { 'content-type': contentType });
      const interval = setInterval(() => {
        response.write(
          contentType === 'text/event-stream' ? ': late\n\n' : ' '
        );
      }, 750);
      response.on('close', () => clearInterval(interval));
    });
  ...
  it('interrupts a slow HTTP header at the absolute request deadline', async () => {
    ...
    const result = await runProxy({
      port: address.port,
      payload: { jsonrpc: '2.0', id: 27, method: 'tools/list' },
      extraEnv: { GBRAIN_MCP_REQUEST_DEADLINE_SECONDS: '1' },
    });
  ...
  it('emits exactly one terminal outcome when responses race the deadline', async () => {
    const port = await listen(async (request, response) => {
      const chunks = [];
      for await (const chunk of request) chunks.push(chunk);
      const payload = JSON.parse(Buffer.concat(chunks).toString('utf8'));
    ...
    for (const id of [28, 29, 30, 31]) {
      expect(responses.filter(response => response.id === id)).toHaveLength(1);
    }
  }, 5000);

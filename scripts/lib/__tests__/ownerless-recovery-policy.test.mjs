...
describe('ownerless recovery policy', () => {
  ...
  it('validates supersession markers, queue ownership, and durable remediation', async () => {
    ...
    expect([{ queued: false }, { queued: false, pending: true }, { queued: true, pending: false }, { queued: false, pending: false }].map(countsAsRecoveryFailure)).toEqual([false, false, false, true]); expect([{ pending: true }, { pending: true, promoted: true }].map(countsAsRecoveryDispatch)).toEqual([true, false]);

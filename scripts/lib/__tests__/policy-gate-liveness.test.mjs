...
describe('runtime recovery liveness', () => {
  it('detects direct and multi-hop recovery cycles', () => {
    ...
    assert.deepEqual(indirect.cycles, [
      ['drain', 'deployment', 'repair', 'drain'],
    ]);
  });

  it('distinguishes progressing waits from stale unknown and expired waits', () => {
    assert.equal(
      evaluateRecoveryLiveness(
        { nodes: [node('drain', { state: 'waiting' })] },
        { now: NOW }
      ).ok,
      true
    );
    ...
    const waiting = evaluateRecoveryLiveness(
      {
        ...
        recoveryPaths,
        workInProgress: [
          { id: 'PR-17433', scopeKey: 'pr:17433', blockedOn: null },
        ],
      },
      { now: NOW }
    );
    assert.equal(waiting.recoveryActions[0].reason, 'unrelated-wip-preserved');
    assert.deepEqual(waiting.cycles, [
      ['repair-17455', 'old-controller', 'repair-17455'],
    ]);

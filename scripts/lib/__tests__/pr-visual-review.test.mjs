...
describe('fail-closed visual evidence gate (JOV-5459)', () => {
  it('treats a missing capture manifest (ENOENT) as failure, not advisory skip', async () => {
    ...
    try {
      ...
      // No manifest.json on disk, evidence is missing.
  ...
  it('exits non-zero from the CLI on missing evidence and records the outcome file', async () => {
    ...
    try {
      ...
      await writeFile(join(dir, 'placeholder.txt'), 'seed');
      ...
      // routing.json is absent, ENOENT means non-zero exit.
      ...
      // A failed stage also exits non-zero even with routing present.
      await writeFile(
      ...
      // Skipped routing with no failed stages passes.
      const skipped = run({});
  ...
  it('keeps the workflow wired to the gate script as the enforcement point', () => {
    ...
    // No job-level continue-on-error on capture (step-level ones remain).

...
export const NATIVE_QUEUE_POLICY = Object.freeze({
  // JOV-5974 (Ops 2026-09-03): live ruleset 10512119 runs the faster knobs
  // build=1 / max_merge=5 / timeout=20m. Enroll preflight fails closed on
  // drift, so this lock must track the live ruleset. Do not restore 3/10/60.
  check_response_timeout_minutes: 20,
  grouping_strategy: 'ALLGREEN',
  // 8→3 on 2026-08-15 after a five-prefix wave showed contention; 3→1 on
  // 2026-09-03 so one combined head owns hosted capacity at a time.
...
export function validateMergeQueueRepoConfig(input) {
  ...
  if (!hasNativeQueue) {
    errors.push('branch-protection.yml must enable GitHub native merge_queue');
  }
  if (
...
export function validateLiveMergeQueueRuleset(ruleset, options = {}) {
  ...
  if (statusRule?.parameters?.strict_required_status_checks_policy !== false) {
    errors.push(
...
/**
 * Batch bisection helper: when a parallel batch fails, isolate the culprit PR
 * and requeue siblings. Uses divide-and-conquer over batchPasses(subset).
 *

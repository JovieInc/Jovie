...
poke, so duplicate CI failure events cannot spawn duplicate Symphony wakes.
"""
...
def test_gate_runs_before_the_paid_poke() -> None:
    workflow = _text()
...
def test_symphony_local_poke_unchanged() -> None:
    workflow = _text()

    assert "Poke Symphony remediator" in workflow
    assert "http://127.0.0.1:4041/api/v1/refresh" in workflow
    assert "symphony-grok-sidecar.service" in workflow
    assert "accountable-writer: Symphony" in workflow

...
const SCALE = [[10,'text-3xs'],[11,'text-2xs'],[12,'text-xs'],[13,'text-app'],
  [14,'text-sm'],[15,'text-mid'],[16,'text-base'],[18,'text-lg'],[20,'text-xl'],
...
let raw = '';
try {
  raw = execSync(`rg -n --no-heading -e 'text-\\[[0-9.]+px\\]' apps/web/components apps/web/app`,
...
for (const ln of raw.split('\n')) {
  ...
  for (const hit of content.matchAll(/text-\[([0-9.]+)px\]/g)) {
    ...
    byPx.get(px).push({ file: file.replace(/^.*apps\/web\//, 'apps/web/'), line, snippet: content.trim().slice(0, 160) });
  }
}

const sizes = [...byPx.keys()].sort((a, b) => byPx.get(b).length - byPx.get(a).length);
const total = [...byPx.values()].reduce((n, a) => n + a.length, 0);
const esc = s => s.replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
const SAMPLE = 'Detroit listeners up 340% — book a show';

const cards = sizes.map(px => {
  const uses = byPx.get(px);
  const { options, tie } = nearest(px);
  const cols = [`<div class="col cur"><span class="tag">current · ${px}px</span><p style="font-size:${px}px">${SAMPLE}</p></div>`]
    .concat(options.map(([p, tok]) => {
      const d = (p - px > 0 ? '+' : '') + (p - px);
      return `<div class="col"><span class="tag">→ ${tok} · ${p}px <em>(${d}px)</em></span><p style="font-size:${p}px">${SAMPLE}</p></div>`;
    }));
  const locs = uses.slice(0, 40).map(u => `<li><code>${esc(u.file)}:${u.line}</code> &nbsp;<span>${esc(u.snippet)}</span></li>`).join('');
  return `<section>
...
const ramp = SCALE.map(([p, t]) => `<span style="font-size:${p}px" title="${t} ${p}px">${p}</span>`).join(' ');

...
generate_env_local() {
  ...
  if [[ ! -d "${REPO_ROOT}/apps/web" ]]; then
    log_warn "Expected apps/web directory not found at ${REPO_ROOT}. Skipping .env.local generation."
    return 0
  fi
  ...
  if ! {
    ...
    log_warn "Failed to write ${env_file}."
    return 0
  fi
...
install_dependencies() {
  ...
  # Navigate to repo root (supports temporary script execution paths)
  REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"

  if [[ ! -f "${REPO_ROOT}/package.json" ]]; then
    log_error "No package.json found at ${REPO_ROOT}. Run this script from a repository worktree root."
    exit 1
  fi

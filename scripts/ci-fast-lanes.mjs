...
function runStructural() {
  ...
  const parts = [
    ...
    'if python3 -c "import coverage, pytest" 2>/dev/null; then COVERAGE_FILE="${RUNNER_TEMP:-/tmp}/jovie-gem-rehabilitation.coverage" python3 -m coverage run --branch scripts/hermes/tests/gem-rehabilitation-policy.test.py && COVERAGE_FILE="${RUNNER_TEMP:-/tmp}/jovie-gem-rehabilitation.coverage" python3 -m coverage report --include="*/scripts/hermes/gem_rehabilitation_policy.py" --fail-under=90 && COVERAGE_FILE="${RUNNER_TEMP:-/tmp}/jovie-gem-gate-next-admission.coverage" python3 -m coverage run --branch scripts/hermes/tests/gem-gate-next-admission.test.py && COVERAGE_FILE="${RUNNER_TEMP:-/tmp}/jovie-gem-gate-next-admission.coverage" python3 -m coverage report --include="*/scripts/hermes/gem-gate-next-admission.py" --fail-under=90 && python3 -m pytest scripts/tests/test_gh_retry.py scripts/tests/test_vercel_prebuilt_deploy.py scripts/tests/test_brand_scrub.py scripts/tests/test_agent_workflow_hygiene.py scripts/tests/test_runner_routing.py scripts/tests/test_symphony_ui_pilot_runtime.py scripts/tests/test_symphony_reconciler_runtime.py -v; else echo "pytest/coverage not installed — skip structural regressions"; fi',
  ];

...
function runStructural() {
  ...
  const operationsParts = [
    ...
    'COVERAGE_FILE="${RUNNER_TEMP:-/tmp}/jovie-gem-repo-drain-cycle.coverage" python3 -m coverage run --branch scripts/symphony/tests/gem-pr-rehabilitation-contract.test.py && COVERAGE_FILE="${RUNNER_TEMP:-/tmp}/jovie-gem-repo-drain-cycle.coverage" python3 -m coverage report --include="*/scripts/symphony/gem-repo-drain-cycle.py" --show-missing --precision=2 --fail-under=80',
